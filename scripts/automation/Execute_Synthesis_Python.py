...
if ($target1Strategy -eq 'identical_merge') {
    ...
    $content1 = Synthesize-Functions $target1Sources $target1Path @('put_major', 'cleanup', 'sig_int', 'get_major') @('upgrade_3_callback', 'sqliteInit', 'sqlite_function_type', 'sqlite_busy_timeout', 'sqliteInitCallback')
}
...
if ($target2Strategy -eq 'identical_merge') {
    ...
    $content2 = Synthesize-Functions $target2Sources $target2Path @('') @('deadlock', 'pick_proc', 'unset_notify_pending', 'mini_notify', 'has_pending')
}
...
if ($target7Strategy -eq 'identical_merge') {
    ...
    $content7 = Synthesize-Functions $target7Sources $target7Path @('') @('libos_msgq_send', 'libos_shm_alloc', 'libos_shm_free', 'libos_sem_post', 'libos_msgq_recv')
}
...
if ($target8Strategy -eq 'identical_merge') {
    ...
    $content8 = Synthesize-Functions $target8Sources $target8Path @('') @('et_getdigit', 'mout')
}
...
if ($target9Strategy -eq 'identical_merge') {
    ...
    $content9 = Synthesize-Functions $target9Sources $target9Path @('Endword', 'Setroman', 'Findchar', 'Findhy', 'Str2word') @('')
}
...
if ($target10Strategy -eq 'identical_merge') {
    ...
    $content10 = Synthesize-Functions $target10Sources $target10Path @('') @('_STUB_malloc', 'free', 'F1', 'F2', 'F0')
}
