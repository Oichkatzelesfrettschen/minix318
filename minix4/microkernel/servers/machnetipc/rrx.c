    ...
    BootId	nrrBid;
{
    ...
    xTrace3(rrxp, TR_EVENTS, "RRX Getting a WRITER LOCK on port %x (readers = %d) (writer = %d)", np->net_port_number, np->rwlock.activeReaders, np->rwlock.activeWriter);
    if ( writerLock(&np->rwlock) == XK_FAILURE ) {
	xTrace0(rrxp, TR_ERRORS, "RRX -- writer lock fails!!");
	xTrace1(rrxp, TR_EVENTS, "RRX Releasing a WRITER LOCK on port %x", np->net_port_number);
	...
	xTrace1(rrxp, TR_EVENTS, "RRX -- aborting due to port state %d",
		np->net_port_type);
	xTrace1(rrxp, TR_EVENTS, "RRX Releasing a WRITER LOCK on port %x", np->net_port_number);
	writerUnlock(&np->rwlock);
	return;
    ...
    XObj	lls;
{
    ...
    switch ( npd->net_port_type ) {
      case MN_FORWARDING:
      case MN_VALID:
    xTrace3(rrxp, TR_EVENTS, "RRX Getting a WRITER LOCK on port %x (readers = %d) (writer = %d)", npd->net_port_number, npd->rwlock.activeReaders, npd->rwlock.activeWriter);
	writerLock(&npd->rwlock);
	if ( npd->net_port_type != MN_INVALID ) {
	    npd->old_net_port_type = npd->net_port_type;
	    xTrace1(rrxp, TR_EVENTS, "RRX Setting mode to MOVING_RECEIVER",
		    npd->net_port_number);
	  ...
	  xTrace1(rrxp, TR_EVENTS, "RRX Releasing a WRITER LOCK on port %x", npd->net_port_number);
    ...
    RrxRepMsg	*rep;
{
	...
	updatePortId(npd, req->u.transfer.msgId, req->u.transfer.orr.h);
    }
