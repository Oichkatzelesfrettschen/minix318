/**
 * @file acpinex_drv.c
 * @brief ACPI nexus driver implementation.
 */
/*
 * CDDL HEADER START
 *
...
int _init(void) {
  ...
  if ((error = ddi_soft_state_init(&acpinex_softstates,
    ...
    cmn_err(CE_WARN, "acpinex: failed to initialize soft state structure.");
    return (error);
  }
...
static int acpinex_info(dev_info_t *dip, ddi_info_cmd_t infocmd, void *arg,
                        void **result) {
  _NOTE(ARGUNUSED(dip));
...
static int acpinex_attach(dev_info_t *devi, ddi_attach_cmd_t cmd) {
  int instance;
  ...
  if (ddi_soft_state_zalloc(acpinex_softstates, instance) != DDI_SUCCESS) {
  ...
  if (ACPI_FAILURE(acpica_get_handle(devi, &softsp->ans_hdl))) {
    ACPINEX_DEBUG(CE_WARN, "!acpinex: failed to get ACPI handle for %s.",
                  softsp->ans_path);
  ...
  (void)ddi_prop_update_string(DDI_DEV_T_NONE, devi, "pm-hardware-state",
                               "no-suspend-resume");
...
static int acpinex_detach(dev_info_t *devi, ddi_detach_cmd_t cmd) {
  int instance;
  acpinex_softstate_t *softsp;
  ...
  if (softsp == NULL) {
    ACPINEX_DEBUG(CE_WARN,
                  ...
                  "object for instance %d in acpinex_detach()",
                  instance);
    return (DDI_FAILURE);
  }
  ...
  case DDI_DETACH:
    ...
    ddi_soft_state_free(acpinex_softstates, instance);
    (void)ddi_prop_update_int(DDI_DEV_T_NONE, devi, DDI_NO_AUTODETACH, 0);
    return (DDI_SUCCESS);
...
static int name_child(dev_info_t *child, char *name, int namelen) {
  char *unitaddr;
 ...
 */
static int acpinex_ctlops(dev_info_t *dip, dev_info_t *rdip, ddi_ctl_enum_t op,
...
static int acpinex_open(dev_t *devi, int flags, int otyp, cred_t *credp) {
  ...
  if (softsp == NULL) {
    ACPINEX_DEBUG(CE_WARN,
                  "!acpinex: failed to get soft state "
                  "object for instance %d in acpinex_open().",
                  instance);
    return (EINVAL);
  }
  ...
  if (ACPINEX_IS_DEVCTL(minor)) {
    ...
    ACPINEX_DEBUG(
        CE_WARN, "!acpinex: invalid minor number %d in acpinex_open().", minor);
    return (EINVAL);
  }
}

static int acpinex_close(dev_t dev, int flags, int otyp, cred_t *credp) {
  _NOTE(ARGUNUSED(flags, otyp, credp));
  ...
  if (softsp == NULL) {
    ACPINEX_DEBUG(CE_WARN,
                  "!acpinex: failed to get soft state "
                  "object for instance %d in acpinex_close().",
                  instance);
    return (EINVAL);
  }
...
static int acpinex_ioctl(dev_t dev, int cmd, intptr_t arg, int mode,
  ...
  if (softsp == NULL) {
    ACPINEX_DEBUG(CE_WARN,
                  "!acpinex: failed to get soft state "
                  "object for instance %d in acpinex_ioctl().",
                  instance);
    return (EINVAL);
  }
 ...
 */
static int acpinex_err_callback(dev_info_t *dip, ddi_fm_error_t *derr,
...
static void acpinex_fm_init(acpinex_softstate_t *softsp) {
  ...
  if (softsp->ans_fm_cap & DDI_FM_ERRCB_CAPABLE) {
     ...
     */
    ddi_fm_handler_register(softsp->ans_dip, acpinex_err_callback, softsp);
  }
}
 ...
 */
static int acpinex_fm_init_child(dev_info_t *dip, dev_info_t *tdip, int cap,
                                 ddi_iblock_cookie_t *ibc) {
  _NOTE(ARGUNUSED(tdip, cap));

  acpinex_softstate_t *softsp =
      ddi_get_soft_state(acpinex_softstates, ddi_get_instance(dip));
