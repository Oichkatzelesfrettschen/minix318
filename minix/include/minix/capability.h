...
// To ensure O_MATHEMATICAL flags are definitely included if fcntl.h above isn't minix/fcntl.h
// This assumes minix/fcntl.h is the final location.
...
/* --- Capability Rights --- */
// These values should align with what the kernel expects (e.g., in capability_syscalls.h or kcapability_posix.h)
// For user-space, these define the API.
...
#define CAP_EXECUTE     0x10  // For exec-like operations if capabilities extend there
...
// Or, they can be negative values returned directly if _syscall convention allows.
// For now, defining them as potential errno values.
...
// This is an opaque handle for user-space. The actual proof structure is kernel-internal.
// It might hold a reference or ID that libc can pass to the kernel (e.g., via m_lc_vfs_cached_cap).
// For now, a simple struct. If it's truly opaque and only passed around, void* might do.
// But the feedback's cap_cache_entry used `capability_t capability;` suggesting it holds something.
typedef struct {
    unsigned long id; // Example: could be a hash or an ID from the kernel
    void *internal_details; // Opaque pointer to more data if needed by libc, not user
} capability_t;
...
/**
 * @brief Enables mathematical verification for subsequent relevant operations in the current process.
 * This typically sets the mode to MATH_MODE_ENABLED.
...
/**
 * @brief Checks if the underlying kernel supports mathematical verification capabilities.
 ...
 * @return The current mode (MATH_MODE_DISABLED, MATH_MODE_ENABLED, MATH_MODE_MANDATORY).
 */
int mathematical_get_mode(void);

/**
 * @brief Checks if mathematical verification was (or would be) applied for an operation.
 * Note: For an existing fd, this is a simplified check based on current global state.
 * A detailed check would require kernel support per fd.
...
/**
 * @brief Opens a file with mathematical verification explicitly requested and required.
 * This is an inline wrapper around open(2) that adds O_MATHEMATICAL and O_MATHEMATICAL_REQUIRED.
 */
 ...
 * These currently use dedicated SYS_MATH_READ, SYS_MATH_WRITE, SYS_MATH_CLOSE syscalls
 * as implemented in mathematical_posix.c. They are not simple flag-based wrappers
 * around the standard read/write/close syscalls.
 * So, they are declared as normal functions.
 */
...
/* (Implemented in libc/sys-minix/mathematical_posix.c using direct kernel syscalls) */

/**
 * @brief Derives a new capability from an existing one (represented by source_fd).
