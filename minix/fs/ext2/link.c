...
PUBLIC int fs_rdlink() {
  ...
  if (r == OK) {
    ...
    r = sys_safecopyto(VFS_PROC_NR, (cp_grant_id_t)fs_m_in.REQ_GRANT,
                       (vir_bytes)0, (vir_bytes)link_text, (size_t)copylen, D);
...
PUBLIC int fs_rename() {
  ...
  if (r == OK) {
    ...
    if (new_ip == NULL) {
      ...
      if (odir &&
          (new_dirp->i_links_count >= SHRT_MAX ||
  ...
  if (r == OK) {
    ...
    if (same_pdir) {
      ...
      r = search_dir(new_dirp, new_name, &numb, ENTER, IGN_PERM,
                     old_ip->i_mode & I_TYPE);
      if (r == OK)
        (void)search_dir(old_dirp, old_name, (ino_t *)0, DELETE, IGN_PERM, 0);
    }
  }
...
off_t newsize;              /* inode must become this size */
{
  ...
  if (newsize > rip->i_size)
    zeroblock_half(rip, rip->i_size, LAST_HALF);
 ...
 *				zeroblock_half				     *
 *===========================================================================*/
PRIVATE void zeroblock_half(rip, pos, half) struct inode *rip;
off_t pos;
int half;
{
 ...
 *				zeroblock_range				     *
 *===========================================================================*/
PRIVATE void zeroblock_range(rip, pos, len) struct inode *rip;
off_t pos;
off_t len;
{
