...
FORWARD _PROTOTYPE(int freesp_inode, (struct inode * rip, off_t st, off_t end));
...
FORWARD _PROTOTYPE(void zerozone_half,
                   (struct inode * rip, off_t pos, int half, int zone_size));
FORWARD _PROTOTYPE(void zerozone_range,
                   (struct inode * rip, off_t pos, off_t len));
...
PUBLIC int fs_rdlink() {
  ...
  if (!S_ISLNK(rip->i_mode))
    ...
    r = sys_safecopyto(VFS_PROC_NR, (cp_grant_id_t)fs_m_in.REQ_GRANT,
                       (vir_bytes)0, (vir_bytes)bp->b_data, (size_t)copylen, D);
...
PUBLIC int fs_rename() {
  ...
  if (r == OK) {
    ...
    if (odir && !same_pdir) {
      ...
      while (TRUE) { /* may hang in a file system loop */
        ...
        next_new_superdirp = advance(new_superdirp, dot2, IGN_PERM);
    ...
    if (new_ip == NULL) {
      ...
      if (odir && new_dirp->i_nlinks >= LINK_MAX && !same_pdir && r == OK) {
        r = EMLINK;
      }
    } else {
      if (old_ip == new_ip)
  ...
  if (r == OK) {
    ...
    if (same_pdir) {
      ...
      if (r == OK)
        (void)search_dir(old_dirp, old_name, NULL, DELETE, IGN_PERM);
    }
  }
 ...
 *				zerozone_half				     *
 *===========================================================================*/
PRIVATE void zerozone_half(rip, pos, half, zone_size) struct inode *rip;
off_t pos;
int half;
int zone_size;
{
 ...
 *				zerozone_range				     *
 *===========================================================================*/
PRIVATE void zerozone_range(rip, pos, len) struct inode *rip;
off_t pos;
off_t len;
{
  ...
  if ((b = read_map(rip, pos)) == NO_BLOCK)
    return;
  while (len > 0) {
