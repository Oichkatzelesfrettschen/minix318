 ...
 * It validates the source and destination addresses, checks process
 * permissions, and performs the actual memory copy using either virtual_copy()
 * or virtual_copy_vmcheck() depending on the flags specified.
 *
 ...
 *   m_lsys_krn_sys_copy.dst_addr		destination offset within
 * segment m_lsys_krn_sys_copy.dst_endpt		destination process
 * number m_lsys_krn_sys_copy.nr_bytes		number of bytes to copy
...
"do_copy: got request from %d (source %d, destination %d)\n",
  ...
  if (m_ptr->m_lsys_krn_sys_copy.flags & CP_FLAG_TRY) {
    ...
    if (r == EFAULT_SRC || r == EFAULT_DST)
      return r = EFAULT; // EFAULT* might be undefined
    if (r == EFAULT_SRC || r == EFAULT_DST)
      return r = EFAULT; // EFAULT* might be undefined
    return r;
  } else {
