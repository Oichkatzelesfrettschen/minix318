...
=======
    ...
    if (RTS_ISSET(rp, RTS_SIGNALED)) {
       ...
       * process. This is a critical security and correctness check (P3). Only
       * the registered signal manager (s_sig_mgr, usually PM) for a target
       * process should be able to retrieve and thus act upon its pending
       * signals. Allowing unauthorized processes to get signals could lead to
       * information leaks or incorrect signal handling.
       */
      KASSERT(caller->p_endpoint == priv(rp)->s_sig_mgr,
              "do_getksig: caller %d is not signal manager %d for target %d",
              caller->p_endpoint, priv(rp)->s_sig_mgr, rp->p_endpoint);
       ...
       * after k_sigemptyset(). This verifies the correctness of k_sigemptyset
       * (P4) and ensures that no signals are inadvertently lost or left
       * unprocessed from this batch. The signal manager is now responsible for
       * all signals that were in 'map'.
       */
      KASSERT(k_sigisempty(&rp->p_pending),
              "do_getksig: signal set p_pending for proc %d (ep %d) not "
              "properly cleared by k_sigemptyset",
