...
#define EMFILE 24
      ...
      static int validate_directory_traversal_mathematical(
          ...
          const char *path);
      static int validate_file_access_mathematical(
          kcapability_dag_t * dag, kcapability_dag_node_t * file_or_dir_cap,
          const char *path, int flags);
      static kcapability_dag_node_t *create_file_capability_mathematical(
          kcapability_dag_t * dag, kcapability_dag_node_t * parent_dir_cap,
          const char *path, int flags, mode_t mode);
      ...
      int do_open_mathematical(struct proc * caller, message * m_ptr) {
        ...
        if (traversal_result != SUCCESS) {
          kprintf_stub("do_open_mathematical: Directory traversal denied for "
                       "path '%s' by caller %d (err: %d)\n",
                       pathname, caller->p_endpoint, traversal_result);
          return traversal_result;
        }
        ...
        if (!(flags & O_CREAT)) { // If not creating, the target must exist
          ...
          if (!target_object_cap) {
            kprintf_stub(
                "do_open_mathematical: Target object '%s' not found under "
                "parent cap ID %llu\n",
                filename,
                (unsigned long long)parent_directory_capability->capability_id);
            return -ENOENT;
        ...
        // If we are not creating and the object doesn't exist
        // (target_object_cap is NULL), it's an error. If we ARE creating,
        // target_object_cap being NULL is expected for a new file. The access
        // check needs to be on parent_dir_cap if creating, or target_object_cap
        // if opening existing.
        ...
        if (flags & O_CREAT && !target_object_cap) {
          ...
          // create_file_capability_mathematical to proceed based on
          // parent_dir_cap. No specific access check on a non-existent file.
          kprintf_stub("do_open_mathematical: O_CREAT set and file does not "
                       "exist. Access check deferred to creation logic.\n");
        } else if (target_object_cap) {
          int access_result = validate_file_access_mathematical(
              kernel_capability_dag, target_object_cap, filename, flags);
          if (access_result != SUCCESS) {
            kprintf_stub("do_open_mathematical: File access denied for '%s', "
                         "flags %x (err: %d)\n",
                         filename, flags, access_result);
            return access_result;
          }
        ...
        if (flags & O_CREAT) {
          final_file_capability = create_file_capability_mathematical(
              kernel_capability_dag, parent_directory_capability, pathname,
              flags, mode);
          if (!final_file_capability) {
            kprintf_stub("do_open_mathematical: Failed to create file "
                         "capability for path '%s'\n",
                         pathname);
            return -ENOMEM;
          }
        } else {
          ...
          KASSERT(final_file_capability != NULL,
                  "do_open_mathematical: target_object_cap is NULL when "
                  "O_CREAT is not set but access passed.");
        }
        ...
        if (fd < 0) {
          kprintf_stub("do_open_mathematical: Failed to allocate FD for file "
                       "capability of path '%s' (err: %d)\n",
                       pathname, fd);
          // If O_CREAT and fd allocation fails, the newly created node/file
          // might need cleanup. This is complex and depends on transactional
          // semantics not yet implemented. For now, if
          // create_file_capability_mathematical added it to DAG, it remains.
          return fd;
        }

        kprintf_stub("do_open_mathematical: Success for path '%s', fd %d "
                     "granted to caller %d\n",
                     pathname, fd, caller->p_endpoint);
        return fd;
      }
      ...
      static kcapability_dag_node_t *lookup_component_capability(
        ...
        for (_BitInt(16) i = 0wb; i < parent_dir_cap->child_count; i++) {
          ...
          if (kstrcmp_c23(component_name, child_node->name) == 0) {
            kprintf_stub("lookup_component_capability: Found component '%s' "
                         "(cap ID %llu) under parent cap ID %llu.\n",
                         component_name,
        ...
        kprintf_stub("lookup_component_capability: Component '%s' not found "
                     "under parent cap ID %llu.\n",
                     component_name,
                     (unsigned long long)parent_dir_cap->capability_id);
        return NULL;
      }
      ...
      static int validate_directory_traversal_mathematical(
        ...
        kprintf_stub(
            "validate_directory_traversal_mathematical: Validating path '%s' "
            "for initial cap ID %llu\n",
            path,
            (unsigned long long)(*current_directory_cap_ptr)->capability_id);
        ...
        // kcapability_dag_node_t* current_directory_cap =
        // *current_directory_cap_ptr; // Local copy for traversal
        ...
        if (path_copy[0] == '/') {
          ...
          // capability. For simplicity, we log and proceed with the provided
          // starting capability.
          kprintf_stub("validate_directory_traversal_mathematical: Absolute "
                       "path, using provided cap as starting point.\n");
        }
        ...
        // component. The loop should consume path components. If a component is
        // the *last* one, it might be a file, not a directory, so traversal
        // checks stop there. The `create_file_capability_mathematical` or
        // `validate_file_access_mathematical` will handle the final component.
        // This function ensures all parent dirs are traversable.
        ...
        while (*next_component_start != '\0') {
          ...
          // itself (which is handled later for access/creation)
          if (next_path_segment != NULL &&
              *next_path_segment != '\0') { // This token is a directory in the
                                            // path, not the final file target
            if (*token ==
                '\0') { // Handles cases like "//" by skipping empty token
              next_component_start = next_path_segment;
              continue;
            ...
            if (!((*current_directory_cap_ptr)->rights_mask &
              ...
              kprintf_stub(
                  "validate_directory_traversal_mathematical: No TRAVERSE "
                  "right in current_directory_cap ID %llu for component '%s'\n",
                  (unsigned long long)(*current_directory_cap_ptr)
                      ->capability_id,
                  token);
              return -EACCES;
            ...
            kcapability_dag_node_t *component_cap = lookup_component_capability(
                dag, *current_directory_cap_ptr, token);

            if (!component_cap) {
              kprintf_stub("validate_directory_traversal_mathematical: "
                           "Component '%s' not found under cap ID %llu\n",
                           token,
                           (unsigned long long)(*current_directory_cap_ptr)
                               ->capability_id);
              return -ENOENT;
            ...
            *current_directory_cap_ptr =
                component_cap; // Update the caller's pointer to the new current
                               // directory
                                       ...
                                       // or final dir)
            kprintf_stub("validate_directory_traversal_mathematical: Reached "
                         "final component '%s', traversal check stops here.\n",
                         token);
            // The rights on this final component will be checked by
            // validate_file_access or create_file_capability
          }
        ...
        kprintf_stub(
            "validate_directory_traversal_mathematical: Path prefix of '%s' "
            "successfully traversed. Final directory cap ID %llu.\n",
            original_path_for_logging,
            (unsigned long long)(*current_directory_cap_ptr)->capability_id);
        return SUCCESS;
      ...
      static int validate_file_access_mathematical(
          kcapability_dag_t * dag,
          kcapability_dag_node_t *
              file_or_dir_cap, // This should be the capability of the actual
                               // file/dir if it exists
          const char *path_or_filename, // Can be full path for logging, or just
                                        // filename
          int flags) {
        KASSERT(dag != NULL, "validate_file_access: dag is NULL");
        KASSERT(file_or_dir_cap != NULL,
                "validate_file_access: file_or_dir_cap is NULL"); // This must
                                                                  // be non-NULL
                                                                  // if checking
                                                                  // existing
        ...
        kprintf_stub("validate_file_access_mathematical: Validating access for "
                     "'%s', flags %x, cap ID %llu, cap rights %llx\n",
                     path_or_filename, flags,
                     (unsigned long long)file_or_dir_cap->capability_id,
                     (unsigned long long)file_or_dir_cap->rights_mask);
        ...
        if ((file_or_dir_cap->rights_mask & required_rights) ==
            required_rights) {
          kprintf_stub("validate_file_access_mathematical: Access GRANTED for "
                       "'%s'. Required: %llx, Has: %llx\n",
                       path_or_filename, (unsigned long long)required_rights,
                       (unsigned long long)file_or_dir_cap->rights_mask);
          return SUCCESS;
        } else {
          kprintf_stub("validate_file_access_mathematical: Access DENIED for "
                       "'%s'. Required: %llx, Has: %llx\n",
                       path_or_filename, (unsigned long long)required_rights,
                       (unsigned long long)file_or_dir_cap->rights_mask);
          return -EACCES;
      ...
      static kcapability_dag_node_t *create_file_capability_mathematical(
        ...
        if (new_file_rights == 0wb &&
            (mode &
             (S_IRUSR |
              S_IWUSR))) { // Log if requested rights became 0 after masking
          kprintf_stub("create_file_capability_mathematical: Calculated "
                       "new_file_rights are 0 for path '%s' mode %o under "
                       "parent cap %llu after parent mask.\n",
                       path, mode,
                       (unsigned long long)parent_dir_cap->capability_id);
        }
        _BitInt(16) new_file_sec_level = parent_dir_cap->security_level;
        ...
        new_file_cap->resource_ptr = (void *)path;
        kprintf_stub(
            "create_file_capability_mathematical: Conceptual: new_file_cap (ID "
            "%llu) for filename '%s' would be added to dag's main store here "
            "if not already handled by node_create.\n",
            (unsigned long long)new_cap_id, filename ? filename : "");
        if (kcapability_dag_add_edge(dag, parent_dir_cap, new_file_cap) !=
            SUCCESS) {
          kprintf_stub("create_file_capability_mathematical: "
        ...
        kprintf_stub("create_file_capability_mathematical: File capability "
                     "created and linked for path '%s', cap ID %llu\n",
                     path, (unsigned long long)new_file_cap->capability_id);
        return new_file_cap;
      }
      ...
      static int allocate_file_descriptor_for_capability(
        ...
        if (fd == -1) {
          kprintf_stub("allocate_fd_for_capability: No free file descriptors "
                       "for caller %d (Max %d)\n",
                       caller->p_endpoint, NO_FILES);
          return -EMFILE;
        }
        kprintf_stub("allocate_fd_for_capability: Conceptually allocated FD %d "
                     "for capability ID %llu for caller %d\n",
                     fd, (unsigned long long)file_cap->capability_id,
                     caller->p_endpoint);
        return fd;
      }
