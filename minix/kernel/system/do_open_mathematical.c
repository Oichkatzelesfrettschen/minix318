...
int do_open_mathematical(struct proc *caller, message *m_ptr) {
  ...
  if (traversal_result != SUCCESS) {
    kprintf_stub("do_open_mathematical: Directory traversal denied for path "
                 "'%s' by caller %d (err: %d)\n",
                 pathname, caller->p_endpoint, traversal_result);
    return traversal_result;
  }

  // parent_directory_capability now holds the capability of the directory where
  // the file is/will be. The actual file/object to check access for is the last
  // component of the path. We need to look up this specific component under
  // parent_directory_capability. If O_CREAT is involved, this component might
  // not exist yet.
  ...
  if (!(flags & O_CREAT)) { // If not creating, the target must exist
    ...
    if (!target_object_cap) {
      kprintf_stub(
          "do_open_mathematical: Target object '%s' not found under parent cap "
          "ID %llu\n",
          filename,
    ...
    // For O_CREAT, target_object_cap might be NULL (new file) or exist (O_EXCL
    // not set). If it exists, validate_file_access_mathematical will check its
    // rights. If it doesn't exist, access check is on parent_dir_cap's ability
    // to allow creation (implicit for now).
    target_object_cap = lookup_component_capability(
        kernel_capability_dag, parent_directory_capability, filename);
    // If target_object_cap is NULL here, it means we are creating a new file.
    // If it's not NULL, we are opening an existing file (possibly with O_TRUNC,
    // etc.).
  }

  // If we are not creating and the object doesn't exist (target_object_cap is
  // NULL), it's an error. If we ARE creating, target_object_cap being NULL is
  // expected for a new file. The access check needs to be on parent_dir_cap if
  // creating, or target_object_cap if opening existing.

  kcapability_dag_node_t *cap_to_check_access_on = target_object_cap;
  if (flags & O_CREAT && !target_object_cap) {
    // For creating a new file, the relevant rights (e.g., WRITE on directory)
    // are checked implicitly by allowing create_file_capability_mathematical to
    // proceed based on parent_dir_cap. No specific access check on a
    // non-existent file.
    kprintf_stub("do_open_mathematical: O_CREAT set and file does not exist. "
    ...
    if (access_result != SUCCESS) {
      kprintf_stub("do_open_mathematical: File access denied for '%s', flags "
                   "%x (err: %d)\n",
    ...
    kprintf_stub(
        "do_open_mathematical: File '%s' does not exist and O_CREAT not set.\n",
        filename);
  ...
  if (flags & O_CREAT) {
    ...
    if (!final_file_capability) {
      kprintf_stub("do_open_mathematical: Failed to create file capability for "
                   "path '%s'\n",
                   pathname);
      return -ENOMEM;
    }
    ...
    KASSERT(final_file_capability != NULL,
            "do_open_mathematical: target_object_cap is NULL when O_CREAT is "
            "not set but access passed.");
  }

  int fd =
      allocate_file_descriptor_for_capability(caller, final_file_capability);
  if (fd < 0) {
    ...
    // need cleanup. This is complex and depends on transactional semantics not
    // yet implemented. For now, if create_file_capability_mathematical added it
    // to DAG, it remains.
    return fd;
  }

  kprintf_stub("do_open_mathematical: Success for path '%s', fd %d granted to "
               "caller %d\n",
...
lookup_component_capability(kcapability_dag_t *dag,
  ...
  for (_BitInt(16) i = 0wb; i < parent_dir_cap->child_count; i++) {
    ...
    if (kstrcmp_c23(component_name, child_node->name) == 0) {
      kprintf_stub("lookup_component_capability: Found component '%s' (cap ID "
                   "%llu) under parent cap ID %llu.\n",
                   component_name,
                   (unsigned long long)child_node->capability_id,
                   (unsigned long long)parent_dir_cap->capability_id);
      return child_node;
    }
  }

  kprintf_stub("lookup_component_capability: Component '%s' not found under "
               "parent cap ID %llu.\n",
               component_name,
               (unsigned long long)parent_dir_cap->capability_id);
  return NULL;
...
static int validate_directory_traversal_mathematical(
  ...
  KASSERT(
      current_directory_cap_ptr != NULL && *current_directory_cap_ptr != NULL,
      "validate_dir_trav: current_directory_cap_ptr is NULL or points to NULL");
  KASSERT(path != NULL, "validate_dir_trav: path is NULL");

  kprintf_stub("validate_directory_traversal_mathematical: Validating path "
  ...
  // kcapability_dag_node_t* current_directory_cap = *current_directory_cap_ptr;
  // // Local copy for traversal

  if (path_copy[0] == '/') {
    // This conceptual implementation does not fully resolve absolute paths from
    // a system root capability. It assumes the initial
    // *current_directory_cap_ptr is either the process's CWD capability or for
    // absolute paths, it should be pre-set to the system's root capability. For
    // simplicity, we log and proceed with the provided starting capability.
    kprintf_stub("validate_directory_traversal_mathematical: Absolute path, "
  ...
  // Path processing logic needs to handle the full path vs. just the next
  // component. The loop should consume path components. If a component is the
  // *last* one, it might be a file, not a directory, so traversal checks stop
  // there. The `create_file_capability_mathematical` or
  // `validate_file_access_mathematical` will handle the final component. This
  // function ensures all parent dirs are traversable.
  ...
  while (*next_component_start != '\0') {
    ...
    // If there's a next segment OR if the current token is not empty (it's the
    // last component) AND this component is not the filename itself (which is
    // handled later for access/creation)
    if (next_path_segment != NULL &&
        *next_path_segment != '\0') { // This token is a directory in the path,
                                      // not the final file target
      if (*token == '\0') { // Handles cases like "//" by skipping empty token
        next_component_start = next_path_segment;
        continue;
      }
      kprintf_stub("validate_directory_traversal_mathematical: Traversing "
                   "component: '%s'\n",
                   token);

      if (!((*current_directory_cap_ptr)->rights_mask & CAP_RIGHT_TRAVERSE)) {
        kprintf_stub(
            "validate_directory_traversal_mathematical: No TRAVERSE right in "
      ...
      kcapability_dag_node_t *component_cap =
          lookup_component_capability(dag, *current_directory_cap_ptr, token);

      if (!component_cap) {
        kprintf_stub(
            "validate_directory_traversal_mathematical: Component '%s' not "
      ...
      *current_directory_cap_ptr =
          component_cap; // Update the caller's pointer to the new current
                         // directory
    } else if (*token !=
               '\0') { // This is the last component (filename or final dir)
      kprintf_stub("validate_directory_traversal_mathematical: Reached final "
  ...
  kprintf_stub("validate_directory_traversal_mathematical: Path prefix of '%s' "
...
static int validate_file_access_mathematical(
    kcapability_dag_t *dag,
    kcapability_dag_node_t *file_or_dir_cap, // This should be the capability of
                                             // the actual file/dir if it exists
    const char
        *path_or_filename, // Can be full path for logging, or just filename
    int flags) {
  KASSERT(dag != NULL, "validate_file_access: dag is NULL");
  KASSERT(
      file_or_dir_cap != NULL,
      "validate_file_access: file_or_dir_cap is NULL"); // This must be non-NULL
                                                        // if checking existing
  KASSERT(path_or_filename != NULL,
          "validate_file_access: path_or_filename is NULL");

  kprintf_stub("validate_file_access_mathematical: Validating access for '%s', "
  ...
  if ((file_or_dir_cap->rights_mask & required_rights) == required_rights) {
    kprintf_stub("validate_file_access_mathematical: Access GRANTED for '%s'. "
                 "Required: %llx, Has: %llx\n",
                 path_or_filename, (unsigned long long)required_rights,
                 (unsigned long long)file_or_dir_cap->rights_mask);
    return SUCCESS;
  } else {
    kprintf_stub("validate_file_access_mathematical: Access DENIED for '%s'. "
                 "Required: %llx, Has: %llx\n",
                 path_or_filename, (unsigned long long)required_rights,
                 (unsigned long long)file_or_dir_cap->rights_mask);
    return -EACCES;
...
create_file_capability_mathematical(kcapability_dag_t *dag,
  ...
  if (new_file_rights == 0wb &&
      (mode &
       (S_IRUSR | S_IWUSR))) { // Log if requested rights became 0 after masking
    kprintf_stub(
        "create_file_capability_mathematical: Calculated new_file_rights are 0 "
        "for path '%s' mode %o under parent cap %llu after parent mask.\n",
  ...
  kprintf_stub("create_file_capability_mathematical: Conceptual: new_file_cap "
               "(ID %llu) for filename '%s' would be added to dag's main store "
  ...
  kprintf_stub("create_file_capability_mathematical: File capability created "
...
allocate_file_descriptor_for_capability(struct proc *caller,
  ...
  if (fd == -1) {
    kprintf_stub("allocate_fd_for_capability: No free file descriptors for "
                 "caller %d (Max %d)\n",
                 caller->p_endpoint, NO_FILES);
    return -EMFILE;
  }
  kprintf_stub("allocate_fd_for_capability: Conceptually allocated FD %d for "
