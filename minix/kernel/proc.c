...
static int  mini_notify(const struct proc *caller, endpoint_t dst, message *user_m_ptr);
 ...
 *  Notification macros                                                      *
 *---------------------------------------------------------------------------*/
/** Zero and setup a notification message for dst_proc from src_proc_nr, including badge. */
#define BuildNotifyMessageWithBadge(msg, src_proc_nr_or_hw, dst_proc_ptr, badge_val) \
    do {                                                            \
        kmemset((msg), 0, sizeof(*(msg)));                          \
        (msg)->m_type                            = NOTIFY_MESSAGE;   \
        /* m_source will be set by the caller of this macro (e.g. mini_receive from pending) */ \
        ...
        if ((src_proc_nr_or_hw) == HARDWARE) {                      \
            ...
            kmemcpy(&(msg)->m_notify.sigset,                         \
                    &priv(dst_proc_ptr)->s_sig_pending,                 \
                    sizeof(k_sigset_t));                            \
 ...
 */
static int mini_notify(const struct proc *caller, endpoint_t dst_e, message *user_m_ptr)
{
    int dst_slot;
    u32_t badge_value = 0;
    ...
    struct proc *dst_ptr = proc_addr(dst_slot); // Renamed to dst_ptr for clarity
    ...
    if (WILLRECEIVE(caller->p_endpoint, dst_ptr, 0, NULL) && // Pass NULL for message, not needed for WILLRECEIVE check
        !(dst_ptr->p_misc_flags & MF_REPLY_PEND)) { // Not busy with SENDREC reply object

        // Build message directly into dst_ptr->p_delivermsg
        BuildNotifyMessageWithBadge(&dst_ptr->p_delivermsg,
                           proc_nr(caller), /* Use proc_nr for src_id type calls */
                           dst_ptr, badge_value);
        dst_ptr->p_delivermsg.m_source = caller->p_endpoint; // Set the true source endpoint
    ...
    // Immediate delivery failed, try to queue using new s_pending_notifications array
    ...
    if (!stored) {
        ...
        kprintf("mini_notify: Pending notification queue for %d full, dropping notification from %d with badge %u\n",
                dst_e, caller->p_endpoint, badge_value);
    }

    // NOTIFY is best-effort, so return OK even if dropped or if it replaced s_notify_pending logic.
    // The old set_sys_bit for s_notify_pending is now fully replaced.
    return OK;
}
...
static int has_pending(sys_map_t *map, int src_id, bool async)
{
    // This function was for the old sys_map_t s_notify_pending and s_asyn_pending.
    // It might need to be adapted or is partially deprecated if s_notify_pending is fully removed.
    // For now, keep its logic for s_asyn_pending if that's still in use.
    // If only s_notify_pending is changed, then this function's use for notifications
    // will be replaced by iterating the new s_pending_notifications array.
    if (src_id != ANY) {
        // Assuming 'map' could be s_asyn_pending. If it was s_notify_pending, this path is dead for notifications.
    ...
    for (int i = 0; i < NR_SYS_PROCS; ++i) {
        if (get_sys_bit(*map, i)) {
            ...
            if (async && RTS_ISSET(p, RTS_VMINHIBIT)) { // async seems to relate to SENDA / s_asyn_pending
...
static void unset_notify_pending(struct proc *caller, int src_id)
{
    ...
    // It should be removed or adapted if a similar "clear one specific source" is needed
    // for the new s_pending_notifications array (which would involve finding it and clearing pn_in_use).
    // For now, commenting out its body as direct bit manipulation is no longer correct for notifications.
    /* unset_sys_bit(priv(caller)->s_notify_pending, src_id); */
    // New logic: Iterate s_pending_notifications, find matching src_id, and clear it.
    // This function might not be directly called anymore if mini_receive clears slots directly.
    ...
    for (int i = 0; i < MAX_PENDING_NOTIFICATIONS; ++i) {
        if (caller_priv->s_pending_notifications[i].pn_in_use &&
            caller_priv->s_pending_notifications[i].pn_source == _ENDPOINT(0, src_id)) { // Assuming src_id is proc_nr
...
// Placeholder for mini_receive modification - this is complex and needs full context
// The diff below will only focus on mini_notify and related helpers for now.
// The actual modification of mini_receive to iterate s_pending_notifications
// will be done in the next diff block.
...
// Note: The old static pm_capability_instance and the cap_lookup stub that were here
// are now removed. The actual cap_lookup is in capability.c and uses per-process tables.
// init_proc_capabilities (in capability.c) sets up initial capabilities.
...
static int ipc_fast_send(struct proc *caller_ptr, capability_t *cap, message *msg) {
    if (!cap) {
    ...
    // streamlined checks and delivery logic, possibly bypassing some parts of mini_send.
    // Here, we'll just print and conceptually call mini_send.
    // kprintf("IPC: ipc_fast_send from %d to %d (proc %d), type %d\n",
    //         caller_ptr->p_endpoint, target_ep, target_proc_nr, msg->m_type);

    // For the getpid() PoC, the target is PM.
    // The actual message delivery for this PoC would be to make PM handle a GETPID request.
    // This simplified ipc_fast_send will just use mini_send.
    // A true "fast path" might avoid some overhead of mini_send if certain conditions are met.
    // (e.g. if receiver is already waiting, copy directly, no full queue/state changes)
    // This is a placeholder for where that optimized logic would go.
    // For now, it demonstrates the capability check and then calls the existing path.
    return mini_send(caller_ptr, target_ep, msg, 0 /* flags */);
}

// New syscall handler for capability-based send
// This would be registered or called from do_ipc if it were a real syscall.
// Parameters: cap_idx (from user), user_msg_ptr (pointer to user message)
// Caller is implicit (current_proc from get_cpulocal_var).
int sys_ipc_send_cap(struct proc *caller_ptr, int cap_idx, message *user_msg_ptr) {
    capability_t *cap;
    ...
    // In MINIX, data_copy is typically used. It's available via syslib.h for user space,
    // but in kernel, direct memory copy or specific kernel_phys_copy might be used if dealing
    // with physical addresses, or direct struct copy if virtual addresses are fine.
    // Let's assume data_copy or equivalent is available/adapted for kernel->kernel or user->kernel.
    // For copying from user caller_ptr to kernel's k_message:
    result = data_copy(caller_ptr->p_endpoint, (vir_bytes)user_msg_ptr,
                      KERNEL, (vir_bytes)&k_message, (phys_bytes)sizeof(message));
    if (result != OK) {
        kprintf("sys_ipc_send_cap: data_copy failed from user %d with error %d\n", caller_ptr->p_endpoint, result);
    ...
    PROF_END(PROF_ID_IPC_MSG_COPY);
    k_message.m_source = caller_ptr->p_endpoint; // Set source endpoint in kernel message copy
    ...
    if (!mdlm_cap_dag_access_check(caller_ptr, cap, OP_SEND)) { // Updated call
...
// This function is intended to be called by a syscall handler for replies in the new IPC mechanism.
// For the getpid() PoC, the app calls SYS_IPC_SEND_CAP, then a regular RECEIVE.
// PM calls a regular send to reply. So this ipc_fast_reply might not be directly
// used by PM in this specific PoC flow, but is part of the conceptual design.
// If the app were to use a new SYS_IPC_RECEIVE_CAP, that new syscall might internally
// note that it's expecting a reply via this path, and a corresponding SYS_IPC_REPLY_CAP
// from PM would then perhaps call ipc_fast_reply.
// For now, it's a conceptual counterpart to ipc_fast_send.
static int ipc_fast_reply(struct proc *replier_ptr, endpoint_t original_caller_ep, message *reply_msg) {
    // kprintf("IPC: ipc_fast_reply from %d to %d, type %d\n",
    //         replier_ptr->p_endpoint, original_caller_ep, reply_msg->m_type);
    ...
    return mini_send(replier_ptr, original_caller_ep, reply_msg, 0 /* flags for blocking send */);
}
