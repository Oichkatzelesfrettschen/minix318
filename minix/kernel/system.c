...
static void kernel_call_finish(struct proc *caller, message *msg, int result) {
  KASSERT(caller != NULL, "kernel_call_finish: caller proc pointer is NULL");
  KASSERT(proc_ptr_ok(caller),
          "kernel_call_finish: caller proc pointer %p not OK", caller);
  KASSERT(msg != NULL,
          "kernel_call_finish: msg pointer is NULL for caller EPT %d",
          caller->p_endpoint);

  if (result == VMSUSPEND) {
    KASSERT(
        RTS_ISSET(caller, RTS_VMREQUEST),
        "kernel_call_finish: VMSUSPEND with no RTS_VMREQUEST for caller EPT %d",
        caller->p_endpoint);
    KASSERT(caller->p_vmrequest.type == VMSTYPE_KERNELCALL,
            "kernel_call_finish: VMSUSPEND with p_vmrequest.type %d != "
            "VMSTYPE_KERNELCALL for caller EPT %d",
            caller->p_vmrequest.type, caller->p_endpoint);
    ...
    if (result != EDONTREPLY) {
      KASSERT(caller->p_delivermsg_vir != NULL,
              "kernel_call_finish: null p_delivermsg_vir for caller EPT %d, "
              "msg type %d, result %d",
 ...
 *
 * Includes assertions for caller validity, message pointer, call number range,
 * privilege mask, and call vector entry existence.
 */
static int kernel_call_dispatch(struct proc *caller, message *msg) {
  ...
  KASSERT(proc_ptr_ok(caller),
          ...
          caller, msg->m_type);
  KASSERT(priv(caller) != NULL,
          "kernel_call_dispatch: null priv structure for caller EPT %d, msg "
          "type %d",
          caller->p_endpoint, msg->m_type);
  KASSERT(msg != NULL,
          "kernel_call_dispatch: msg pointer is NULL for caller EPT %d",
          caller->p_endpoint);
  ...
  KASSERT(call_nr >= 0 && call_nr < NR_SYS_CALLS,
          ...
          "(0-%d) for caller EPT %d",
          call_nr, msg->m_type, NR_SYS_CALLS - 1, caller->p_endpoint);

#if DEBUG_IPC_HOOK
  hook_ipc_msgkcall(msg, caller);
#endif
  // This KASSERT makes the privilege check fatal in debug builds.
  KASSERT(GET_BIT(priv(caller)->s_k_call_mask, call_nr),
          "kernel_call_dispatch: call %d (type %d) denied by s_k_call_mask for "
          "caller EPT %d",
  ...
  // This KASSERT makes an unimplemented call fatal in debug builds.
  KASSERT(
      call_vec[call_nr] != NULL,
      "kernel_call_dispatch: call_vec[%d] (type %d) is NULL for caller EPT %d",
      call_nr, msg->m_type, caller->p_endpoint);
  if (!call_vec[call_nr]) {
...
void kernel_call(message *m_user, struct proc *caller) {
  ...
  KASSERT(proc_ptr_ok(caller), "kernel_call: caller proc pointer %p not OK",
          caller);
  KASSERT(m_user != NULL,
          "kernel_call: received null message pointer from userspace for "
          "caller EPT %d",
          caller->p_endpoint);
  ...
  KASSERT(msg.m_source == caller->p_endpoint,
          "kernel_call: internal m_source assignment error for caller EPT %d",
          caller->p_endpoint);
...
void system_init(void) {
  ...
  map(SYS_MATH_OPEN, do_open_mathematical); /* Mathematical open */
}
