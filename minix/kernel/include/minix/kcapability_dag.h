 ...
 * - Insert: O(V+E) in worst case (rebuilding lists), O(1) amortized if
 * well-managed
 * - Insert: O(V+E) in worst case (rebuilding lists), O(1) amortized if
 * well-managed
...
typedef struct kcapability_dag_node {
  ...
  _BitInt(64) rights_mask;   /**< Bitmask of permissions/rights granted by this
                                capability. */
  _BitInt(16)
      security_level; /**< Security level associated with this capability. */
  _BitInt(32)
      generation; /**< Generation count for ABA problem mitigation with IDs. */
  _BitInt(16) reference_count; /**< Number of references to this node; aids in
  ...
  void *resource_ptr; /**< Opaque pointer to the underlying kernel resource or
                         object this capability pertains to. */
  void *resource_ptr; /**< Opaque pointer to the underlying kernel resource or
                         object this capability pertains to. */

  struct kcapability_dag_node *
      *parents; /**< Dynamically allocated array of pointers to parent
                   capability nodes. */
  ...
  struct kcapability_dag_node *
      *children;           /**< Dynamically allocated array of pointers to child
                              capability nodes. */
  _BitInt(16) child_count; /**< Current number of child nodes. */
  _BitInt(16)
      children_capacity; /**< Allocated capacity of the children array. */
  struct kcapability_dag_node *
      *children;           /**< Dynamically allocated array of pointers to child
                              capability nodes. */
  _BitInt(16) child_count; /**< Current number of child nodes. */
  ...
  _BitInt(8) cached_validation_result; /**< Cached result of a previous
                                          validation (e.g., VALIDATION_UNKNOWN,
                                          VALID, INVALID). */
  _BitInt(64) cache_timestamp;         /**< Timestamp of when the
                                          cached_validation_result was stored. */
  /* Performance and validation cache fields */
  _BitInt(8) cached_validation_result; /**< Cached result of a previous
                                          validation (e.g., VALIDATION_UNKNOWN,
                                          VALID, INVALID). */
  ...
  /* Consider: __attribute__((aligned(64))) for cache line optimization if
   * critical. */
  /* Consider: __attribute__((aligned(64))) for cache line optimization if
   * critical. */

  // Fields for identifying process-specific capability nodes
  _BitInt(1) is_process_main_cap_node; /**< Flag: True if this node represents a
                                          process's primary capability set. */
  endpoint_t owner_endpoint; /**< If is_process_main_cap_node is true, this
                                holds the process's endpoint. */

  char name[MAX_COMPONENT_NAME_LEN]; /**< Name of the component if this node
                                        represents a named entity like a
                                        file/directory. */
...
typedef struct kcapability_dag {
   ...
   * Option 1: A flat array or hash table (implementation detail not shown
   * here). Option 2: Pointers to root nodes (if the DAG has specific roots).
   * For this template, we'll keep it abstract. 'nodes' could represent
   * the primary access point to the node collection.
   */
  kcapability_dag_node_t *nodes; /**< Pointer to an array of nodes, acting as
                                    the primary node store. */
  _BitInt(32)
      node_count; /**< Current total number of active nodes in the DAG. */
  _BitInt(32)
      node_capacity; /**< Current allocated capacity of the 'nodes' array. */
  ...
  bool (*validate_invariants)(
      const struct kcapability_dag
          *dag); /**< Pointer to a function that validates DAG invariants. */
  // int (*compare_nodes)(const kcapability_dag_node_t* a, const
  // kcapability_dag_node_t* b); void (*traverse_dag)(const struct
  // kcapability_dag* dag, void (*visit_func)(kcapability_dag_node_t* node,
  // void* context), void* context);

  /** Performance monitoring for DAG operations. */
  struct {
    _BitInt(64) operations_total; /**< Total number of significant DAG
                                     operations (insert, delete, lookup). */
    _BitInt(64)
        cache_hits; /**< Cache hits during node lookups (if applicable). */
    _BitInt(64)
        cache_misses; /**< Cache misses during node lookups (if applicable). */
    _BitInt(64)
        traversals; /**< Number of full or partial traversals performed. */
    _BitInt(64) successful_derivations; /**< Count of successful derivation
                                           validations. */
    _BitInt(64)
        failed_derivations; /**< Count of failed derivation validations. */
  } performance_stats;
...
/**
 * @brief Creates and initializes an empty Capability Directed Acyclic Graph
 * (DAG).
 * @param initial_node_pool_capacity Hint for the initial capacity of any
 * internally managed node storage. Actual allocation strategy is
 * implementation-defined.
 * @return Pointer to the newly created kcapability_dag_t, or NULL on failure
 ...
 *
 * The node is initialized with the given parameters. Its generation is
 * typically set to an initial value (e.g., 0 or 1), and its reference_count
 * might be initialized to 1 (if the creator holds a reference) or 0. The
 * children array is typically initialized to NULL with zero capacity/count.
 *
 * @param id The unique identifier for the new node.
 * @param granted_permissions A bitmask of permissions granted by this
 * capability node.
 * @param resource_ptr Pointer to the underlying kernel resource this capability
 * governs.
 * @param name_param The name for this capability node (e.g. filename). Can be
 * NULL.
 * @return Pointer to the newly created kcapability_dag_node_t, or NULL on
 * failure.
 */
 ...
 *         -EEXIST if a node with the same ID already exists, -ENOMEM if
 * internal reallocation fails).
 */
int kcapability_dag_insert_node(kcapability_dag_t *dag,
 ...
 * - Whether parent_node and child_node are already part of the DAG (managed by
 * node_store_handle).
 * - Potential cycles if strict DAG properties must be maintained dynamically
 * (can be complex). Alternatively, cycle detection can be part of
 * `kcapability_dag_validate_invariants`.
 *
 * @param dag Pointer to the kcapability_dag_t structure (used for context,
 ...
 * @return 0 on success, or an error code (e.g., -EINVAL, -ENOMEM, -ELOOP if
 * cycle detected and disallowed).
 */
 ...
 * @return Pointer to the found kcapability_dag_node_t, or NULL if not found or
 * if `dag` is NULL.
 */
kcapability_dag_node_t *
kcapability_dag_lookup_node(const kcapability_dag_t *dag, _BitInt(64) id);
 ...
 * The node's memory itself is not freed here unless its reference count drops
 * to zero (if ref counting is the primary lifecycle mechanism). See
 * `kcapability_dag_node_destroy`.
 *
 * @param dag Pointer to the kcapability_dag_t structure.
 ...
 * @param parent_node Pointer to the parent kcapability_dag_node_t.
 * @param child_node Pointer to the child kcapability_dag_node_t whose edge from
 * parent_node should be removed.
 * @return 0 on success, or an error code (e.g., -ENOENT if the edge doesn't
 * exist).
 */
 ...
 * potentially by calling `kcapability_dag_node_destroy` on each) and the DAG
 * structure itself.
 *
