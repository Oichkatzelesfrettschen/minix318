...
#include <klib.h>  // For kcpu_detect_features, kprintf_stub, etc.
...
void bsp_finish_booting(void) {
  ...
  if (boot_cpu_init_timer(system_hz)) {
    panic(
        "FATAL : failed to initialize timer interrupts, "
...
void kmain(kinfo_t *local_cbi) {
  ...
  // Detect CPU features early in the kernel initialization.
  kcpu_detect_features();
  kprintf_stub("CPU features detected.\n");  // MODIFIED

  // Run klib mathematical foundation tests (e.g., DAG tests)
  ...
  for (i = 0; i < NR_BOOT_PROCS; ++i) {
    ...
    if (schedulable_proc) {
      ...
      if (ipc_to_m == ALL_M) {
        for (j = 0; j < NR_SYS_PROCS; j++) set_sys_bit(map, j);
      }
    ...
    if (!get_cpulocal_var(proc_ptr)) get_cpulocal_var(proc_ptr) = rp;
...
void cstart(void) {
  ...
  if ((value = env_get(VERBOSEBOOTVARNAME)))
    verboseboot = 0;
        /* FIXME: atoi(value) was here, replace with katoi */  // MODIFIED
  verboseboot = 0;
      /* FIXME: atoi(value) was here, replace with katoi */  // MODIFIED
  ...
  if (value && (*value != '0')) {
    /* FIXME: atoi(value) was here, simple check for non-zero for now */  // MODIFIED
    if (value && (*value != '0')) {
      /* FIXME: atoi(value) was here, simple check for non-zero for now */  // MODIFIED
    ...
    if (value)
      config_no_apic = (*value != '0');
          /* FIXME: atoi(value) was here */  // MODIFIED (default to true if
                                             // value is non-zero)
    config_no_apic = (*value != '0');
        /* FIXME: atoi(value) was here */  // MODIFIED (default to true if value
                                           // is non-zero)
    else config_no_apic = 1;               /* Default if not set */
    config_no_apic = 1;                    /* Default if not set */
    value = env_get("apic_timer_x");
    if (value)
      config_apic_timer_x = 0;
          /* FIXME: atoi(value) was here, replace with katoi */  // MODIFIED
                                                                 // (default to
                                                                 // 0, needs
                                                                 // proper
    ...
    config_apic_timer_x = 0;
        /* FIXME: atoi(value) was here, replace with katoi */  // MODIFIED
                                                               ...
                                                               // needs proper
                                                               // katoi)
    else config_apic_timer_x = 1; /* Default if not set */
...
#ifdef CONFIG_SMP
    if (config_no_apic) config_no_smp = 1;
    value = env_get("no_smp");
    if (value)
      config_no_smp = (*value != '0');
          /* FIXME: atoi(value) was here */  // MODIFIED (default to true if
                                             // value is non-zero)
    config_no_smp = (*value != '0');
        /* FIXME: atoi(value) was here */  // MODIFIED (default to true if value
                                           // is non-zero)
    else config_no_smp = 0;                /* Default if not set */
