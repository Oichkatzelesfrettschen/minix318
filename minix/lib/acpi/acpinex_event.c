/**
 * @file acpinex_event.c
 * @brief ACPI nexus event handling.
 */
 ...
 */
static int acpinex_event_generate_event(dev_info_t *dip, ACPI_HANDLE hdl,
                                        int req, int event, char *objname) {
  int rv = 0;
  sysevent_id_t eid;
  ...
  if (ACPI_FAILURE(
          acpidev_dr_get_attachment_point(hdl, attach_pnt, MAXPATHLEN))) {
    cmn_err(CE_WARN, "!acpinex: failed to generate AP name for %s.", objname);
  ...
  if (rv != 0) {
    cmn_err(CE_WARN, "!acpinex: failed to add attr [%s] for %s event.",
            DR_AP_ID, EC_DR);
    kmem_free(attach_pnt, MAXPATHLEN);
    return (rv);
  }
  ...
  rv = sysevent_add_attr(&evnt_attr_list, DR_REQ_TYPE, &evnt_val, KM_SLEEP);
  if (rv != 0) {
    cmn_err(CE_WARN, "!acpinex: failed to add attr [%s] for %s event.",
            DR_REQ_TYPE, EC_DR);
  ...
  case ACPI_NOTIFY_EJECT_REQUEST:
    ...
    break;
  default:
    cmn_err(CE_WARN, "!acpinex: unknown ACPI event type %d.", event);
    sysevent_free_attr(evnt_attr_list);
    kmem_free(attach_pnt, MAXPATHLEN);
    return (-1);
  }
  ...
  rv = ddi_log_sysevent(dip, DDI_VENDOR_SUNW, EC_DR, ESC_DR_REQ, evnt_attr_list,
                        &eid, KM_SLEEP);
  if (rv != DDI_SUCCESS) {
    cmn_err(CE_WARN, "!acpinex: failed to log DR_REQ event for %s.", objname);
    rv = -1;
  }
 ...
 */
static void acpinex_event_handle_eject_request(ACPI_HANDLE hdl,
  ...
  if (sp == NULL || sp->ans_dip == NULL || sp->ans_hdl == NULL) {
    ...
    ACPINEX_DEBUG(CE_WARN, "!acpinex: softstate data structure is invalid.");
    cmn_err(CE_WARN, "!acpinex: failed to handle EJECT_REQUEST event from %s.",
            objname);
  ...
  if (acpinex_event_support_remove == 0) {
    cmn_err(CE_WARN, "!acpinex: hot-removing of device %s is unsupported.",
            objname);
    code = ACPI_OST_STA_EJECT_NOT_SUPPORT;
  } else if (acpinex_event_generate_event(sp->ans_dip, hdl, SE_OUTGOING_RES,
                                          ACPI_NOTIFY_EJECT_REQUEST,
                                          objname) != 0) {
    cmn_err(CE_WARN,
            "!acpinex: failed to generate ESC_DR_REQ "
            "event for device eject request from %s.",
            objname);
    code = ACPI_OST_STA_FAILURE;
  } else {
    cmn_err(CE_NOTE,
            "!acpinex: generate ESC_DR_REQ event for "
            "device eject request from %s.",
            objname);
    code = ACPI_OST_STA_EJECT_IN_PROGRESS;
  }
  if (call_ost) {
    (void)acpidev_eval_ost(hdl, ACPI_NOTIFY_EJECT_REQUEST, code, NULL, 0);
  }
...
static ACPI_STATUS acpinex_event_handle_check_one(ACPI_HANDLE hdl, UINT32 lvl,
  ...
  if (dhdl == NULL) {
    ...
    ACPINEX_DEBUG(CE_NOTE, "!acpinex: failed to get data associated with %p.",
                  hdl);
  ...
  case 0x1:
    /* Surprising removal. */
    cmn_err(CE_WARN, "!acpinex: device %s has been surprisingly removed.",
            objname);
  ...
  case 0x2:
    /* Hot-adding. */
    ACPINEX_DEBUG(CE_NOTE, "!acpinex: device %s has been inserted.", objname);
    argp->device_insert++;
    if (acpinex_event_generate_event(argp->softstatep->ans_dip, hdl,
                                     SE_INCOMING_RES, argp->event_type,
                                     objname) != 0) {
      cmn_err(CE_WARN,
              "!acpinex: failed to generate ESC_DR_REQ event for "
              "device insert request from %s.",
              objname);
      argp->device_fail++;
    } else {
      cmn_err(CE_NOTE,
              "!acpinex: generate ESC_DR_REQ event "
              "for device insert request from %s.",
              objname);
    }
    acpidev_free_object_name(objname);
    return (AE_OK);
 ...
 * It also invokes ACPI _OST method to update event status if call_ost is true.
 */
static void acpinex_event_handle_check_request(int event, ACPI_HANDLE hdl,
                                               acpinex_softstate_t *sp,
                                               boolean_t call_ost) {
  ACPI_STATUS rv;
  int code;
  ...
  if (sp == NULL || sp->ans_dip == NULL || sp->ans_hdl == NULL) {
    if (call_ost) {
      (void)acpidev_eval_ost(hdl, event, ACPI_OST_STA_FAILURE, NULL, 0);
    }
    ACPINEX_DEBUG(CE_WARN, "!acpinex: softstate data structure is invalid.");
    cmn_err(CE_WARN,
            "!acpinex: failed to handle "
            "BUS/DEVICE_CHECK event from %s.",
            objname);
    acpidev_free_object_name(objname);
    return;
  }
  ...
  if (ACPI_SUCCESS(rv)) {
    rv = AcpiWalkNamespace(ACPI_TYPE_DEVICE, hdl, ACPIDEV_MAX_ENUM_LEVELS,
                           &acpinex_event_handle_check_one, NULL, &arg, NULL);
  }

  if (ACPI_FAILURE(rv)) {
    /* Failed to scan the ACPI namespace. */
    cmn_err(CE_WARN, "!acpinex: failed to handle event %d from %s.", event,
            objname);
    code = ACPI_OST_STA_FAILURE;
    ...
    ACPINEX_DEBUG(CE_WARN, "!acpinex: failed to check status of some devices.");
    code = ACPI_OST_STA_FAILURE;
  } else if (arg.device_insert == 0) {
    /* No hot-added devices found. */
    cmn_err(CE_WARN, "!acpinex: no hot-added devices under %s found.", objname);
    code = ACPI_OST_STA_FAILURE;
  } else {
...
static void acpinex_event_system_handler(ACPI_HANDLE hdl, UINT32 type,
                                         void *arg) {
  acpinex_softstate_t *sp;
  ...
  default:
    ACPINEX_DEBUG(CE_NOTE, "!acpinex: unhandled event(%d) on hdl %p under %s.",
                  type, hdl, sp->ans_path);
    (void)acpidev_eval_ost(hdl, type, ACPI_OST_STA_NOT_SUPPORT, NULL, 0);
    break;
  }
 ...
 */
static int acpinex_event_install_handler(ACPI_HANDLE hdl, void *arg,
  ...
  if (status == AE_OK || status == AE_ALREADY_EXISTS) {
    ...
    objname = acpidev_get_object_name(hdl);
    cmn_err(CE_WARN, "!acpinex: failed to install system event handler for %s.",
            objname);
    acpidev_free_object_name(objname);
    rc = DDI_FAILURE;
  }
 ...
 * Return DDI_SUCCESS on success, and DDI_FAILURE on failure.
 */
static int acpinex_event_uninstall_handler(ACPI_HANDLE hdl,
                                           ACPI_DEVICE_INFO *infop,
                                           acpidev_data_handle_t dhdl) {
  ASSERT(hdl != NULL);
  ASSERT(dhdl != NULL);
  ...
  if (ACPI_SUCCESS(AcpiRemoveNotifyHandler(hdl, ACPI_SYSTEM_NOTIFY,
    ...
    objname = acpidev_get_object_name(hdl);
    cmn_err(CE_WARN,
            "!acpinex: failed to uninstall system event "
            "handler for %s.",
            objname);
    acpidev_free_object_name(objname);
    return (DDI_FAILURE);
 ...
 * Return DDI_SUCCESS on success, and DDI_FAILURE on failure.
 */
static int acpinex_event_walk(boolean_t init, acpinex_softstate_t *sp,
                              ACPI_HANDLE hdl) {
  int rc;
  ...
  ASSERT(hdl != NULL);
  while (ACPI_SUCCESS(AcpiGetNextObject(ACPI_TYPE_ANY, hdl, child, &child))) {
    ...
    if (dhdl == NULL) {
      ACPINEX_DEBUG(CE_NOTE,
                    "!acpinex: failed to get data "
                    "associated with %p, skip.",
                    child);
      continue;
    }
    ...
    if (ACPI_FAILURE(AcpiGetObjectInfo(child, &infop))) {
      cmn_err(CE_WARN, "!acpidnex: failed to get object info for %p.", child);
      continue;
    }

    if (init) {
      rc = acpinex_event_install_handler(child, sp, infop, dhdl);
      if (rc != DDI_SUCCESS) {
        ACPINEX_DEBUG(CE_WARN,
                      "!acpinex: failed to "
         ...
         */
      } else if (acpidev_check_device_enabled(acpidev_data_get_status(dhdl)) &&
      ...
      if (rc == DDI_SUCCESS) {
        rc = acpinex_event_uninstall_handler(child, infop, dhdl);
      }
...
int acpinex_event_scan(acpinex_softstate_t *sp, boolean_t init) {
  int rc;
  ...
  if (rc != DDI_SUCCESS) {
    if (init) {
      ACPINEX_DEBUG(CE_WARN,
                    "!acpinex: failed to "
                    "configure child objects of %s.",
                    sp->ans_path);
      rc = DDI_FAILURE;
    } else {
      ACPINEX_DEBUG(CE_WARN,
                    "!acpinex: failed to "
                    "unconfigure child objects of %s.",
                    sp->ans_path);
