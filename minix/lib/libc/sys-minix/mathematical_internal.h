...
struct internal_capability {
    capability_t public_cap;        /* The user-visible part (could be an ID or small data) */
    ...
    unsigned char proof_hash[32];   /* SHA-256 of the proof, if proofs are passed to/cached in userspace */
...
enum math_mode {
    ...
    MATH_MODE_MANDATORY = 2,       /* Always verify; fail operations if verification not possible or fails */
    MATH_MODE_AUDIT = 3,           /* Verify and log everything (could be combined with ENABLED/MANDATORY) */
};

/* Kernel policy flags - these map to kernel-side definitions from SYS_CAPABILITY_QUERY */
#define POLICY_AVAILABLE        0x0001  // Kernel supports the capability system
#define POLICY_MANDATORY_ALL    0x0002  // Kernel mandates verification for all processes
#define POLICY_MANDATORY_PRIV   0x0004  // Kernel mandates for privileged processes (e.g. root)
#define POLICY_CACHE_KERNEL     0x0008  // Kernel suggests/supports kernel-side caching (info only for libc)
...
struct math_state {
    ...
    /* Cache management */
    int cache_enabled;
    struct cap_cache_entry **cache_hash_table; // Hash table (array of pointers to list heads)
    ...
    int kernel_notify_fd;          /* File descriptor for kernel events (if using select/poll) */

    /* Debugging and diagnostics */
    int debug_level;
    FILE *audit_log_file;          // Changed from audit_log to avoid conflict with potential function
};
...
struct thread_math_state {
    ...
    /* Thread-local capability cache for very frequent operations (e.g. last N capabilities) */
...
enum math_mode __get_effective_mode(void); // Gets current effective mode (global or thread)
int __should_use_mathematical(int flags);  // Decides if math verification should be attempted for an op
struct thread_math_state *__get_thread_state(int create_if_missing); // Manages TLS

/* Capability cache management (from mathematical_cache.c or similar) */
void __init_capability_cache_subsystem(void); // Initializes __math_state.cache_hash_table etc.
...
void __invalidate_path_capabilities(const char *path); // Example for more specific invalidation

/* Kernel communication (from mathematical_kernel_comm.c or similar) */
int __query_kernel_policy(unsigned int *policy_flags_from_kernel); // Fills policy_flags_from_kernel
int __notify_kernel_verification(const char *path, int result_code); // Informs kernel about a verification result
int __register_kernel_notifications(void); // Sets up kernel_notify_fd if used

/* Proof construction and verification - internal versions (may call kernel via syscalls) */
/* These are internal libc wrappers that might call SYS_CAP_CONSTRUCT etc. */
/* The capability_proof struct is from the kernel's kcapability_posix.h, assumed opaque here if passed. */
struct capability_proof;
struct capability_proof *__construct_capability_proof_libc(endpoint_t proc_ep, const char *path, int rights);
...
/* Debugging and diagnostics */
void __log_capability_operation(const char *op, const char *path, int result, const char *detail);
...
int __validate_capability_transition(const capability_t *from_cap, const capability_t *to_cap, int operation_type);
int __check_capability_tampering(const struct internal_capability *cap_internal);

/* Performance optimizations */
static inline int __quick_capability_check(int flags) {
    if (!(flags & (O_MATHEMATICAL | O_MATHEMATICAL_REQUIRED))) // O_flags from fcntl.h
        return 0; // Not a mathematical open request
    if (!__math_state.initialized)
        return 0; // Libc state not ready
    if (__math_state.mode == MATH_MODE_DISABLED && !(__math_state.policy & (POLICY_MANDATORY_ALL | POLICY_MANDATORY_PRIV)))
        return 0; // Disabled and not forced by kernel policy
    return 1; // Potentially needs math check
}

/* Internal Error codes for libc internal use, translated to errno for public API */
...
#define MATH_ERR_KERNEL_COMM       (MATH_ERR_BASE - 2) // General kernel communication error
#define MATH_ERR_PROOF_EXPIRED     (MATH_ERR_BASE - 3) // If userspace caches and validates proofs
#define MATH_ERR_TAMPERING         (MATH_ERR_BASE - 4) // If a cached proof/cap looks tampered
#define MATH_ERR_NO_KERNEL_SUPPORT (MATH_ERR_BASE - 5) // Kernel doesn't support feature but was required
#define MATH_ERR_INVALID_ARG       (MATH_ERR_BASE - 6) // Bad arguments to internal functions
...
/* These define the payload for messages if specific structures are used for kernel syscalls */
/* For SYS_CAPABILITY_QUERY, the kernel just returns flags in m1_i1 (current plan) */
