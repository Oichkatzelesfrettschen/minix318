...
#define KCAPABILITY_MAX_EDGES_PER_NODE 8  // Default max edges per node (renamed to children)
#endif

#ifndef KCAPABILITY_MAX_PARENTS_PER_NODE
#define KCAPABILITY_MAX_PARENTS_PER_NODE 8 // Default max incoming edges (parents)
#endif
 ...
 * @brief Defines a type for timestamps, e.g., for cache validation or generation counts.
 * Using C23 _BitInt for precise width, representing e.g., nanoseconds or ticks.
 */
...
typedef enum kcap_validation_state_t {
    KCAP_VALIDATION_UNKNOWN = 0, /**< Validation state not yet determined or cache stale. */
    KCAP_VALIDATION_VALID = 1,   /**< Node has been validated as part of a legitimate derivation. */
 ...
 * Aligned to cache line size for performance (K_CACHE_LINE_SIZE assumed defined elsewhere).
 */
typedef struct kcapability_dag_node_s {
    kcapability_id_t id;            /**< Unique identifier for this capability node. */
    krights_mask_t rights_mask;     /**< Bitmask of rights this capability possesses. */
    ksecurity_level_t security_level;/**< Security level of this capability (use _BitInt(16)). */
    _BitInt(32) generation;         /**< Generation count for ABA problem mitigation in dynamic scenarios. */
    kbool is_active;                /**< Flag to indicate if this node slot is in use (for static array model). */

    // Children (outgoing edges) - Adjacency list
    struct kcapability_dag_edge_t children[KCAPABILITY_MAX_EDGES_PER_NODE];
    _BitInt(KCAPABILITY_MAX_EDGES_PER_NODE > 0 ? (unsigned int)__builtin_clz(0) - (unsigned int)__builtin_clz(KCAPABILITY_MAX_EDGES_PER_NODE - 1) : 1) num_children;

    // Parents (incoming edges) - Reverse adjacency list
    struct kcapability_dag_node_s* parents[KCAPABILITY_MAX_PARENTS_PER_NODE];
     _BitInt(KCAPABILITY_MAX_PARENTS_PER_NODE > 0 ? (unsigned int)__builtin_clz(0) - (unsigned int)__builtin_clz(KCAPABILITY_MAX_PARENTS_PER_NODE - 1) : 1) num_parents;

    // Validation cache
    kcap_validation_state_t cached_validation_result; /**< Cached result of a previous validation. */
    k_timestamp_t cache_timestamp;                    /**< Timestamp of when this result was cached. */

    // Temporary flags for DFS traversal (note: for SMP, pass context or use generation counts)
    kbool visited_flag;             /**< Temp flag for DFS: visited in current traversal. */
    kbool recursion_stack_flag;     /**< Temp flag for DFS: currently in recursion stack. */
} __attribute__((aligned(64))) kcapability_dag_node_t; // Assuming K_CACHE_LINE_SIZE = 64
...
typedef struct kcap_dag_perf_stats_t {
    ...
    _BitInt(64) cache_hits;            /**< Number of cache hits during lookups (conceptual). */
    _BitInt(64) cache_misses;          /**< Number of cache misses during lookups (conceptual). */
    _BitInt(64) successful_derivations;/**< Number of successful derivation validations. */
    _BitInt(64) failed_derivations;    /**< Number of failed derivation validations. */
 ...
 * This implementation uses a statically sized array for nodes within the DAG structure.
 */
typedef struct kcapability_dag_t {
    kcapability_dag_node_t nodes[KCAPABILITY_MAX_NODES]; /**< Static array of nodes. */
    // num_nodes uses a bit-width appropriately sized for KCAPABILITY_MAX_NODES
    _BitInt(KCAPABILITY_MAX_NODES > 0 ? (unsigned int)__builtin_clz(0) - (unsigned int)__builtin_clz(KCAPABILITY_MAX_NODES - 1) : 1) num_nodes; /**< Current number of active nodes. */
    _BitInt(32) edge_count; /**< Current number of active edges in the DAG. */

    kcap_dag_perf_stats_t performance_stats; /**< Performance counters for DAG operations. */
...
kcapability_dag_node_t* kcapability_dag_lookup_node(
    kcapability_dag_t* dag, kcapability_id_t capability_id);
...
// Edge management
kcapability_dag_status_t kcapability_dag_add_edge(
