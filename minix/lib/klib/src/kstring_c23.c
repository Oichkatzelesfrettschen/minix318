 ...
 * as part of the MINIX kernel library. The implementation leverages C23 language
 * features for enhanced type safety and potential performance optimization.
 *
 ...
 * @pre s != NULL
 *
 * @post The number of characters that precede the terminating null character is returned.
 *
 * @invariant Not specified
 *
 * @note The behavior is undefined if 's' does not point to a null-terminated string
 *       or if 's' is an invalid pointer.
 *
 * @warning Reading from an invalid or non-null-terminated string can lead to crashes.
 *
...
size_t kstrlen_c23(const char *s) {
    ...
    /*     return kstrlen_c23_optimized(s); */ /* Pass 's' if optimized function needs it */
    /* } */
 ...
 * The 'restrict' keyword is used for `dest` and `src` to aid compiler optimization,
 * indicating no aliasing between these pointers.
 *
 * Algorithm Complexity: O(n), where n is min(strlen(src), size-1).
 * Memory Requirements: O(1)
 *
 * @param[out] dest Pointer to the destination buffer. Must not be NULL if size > 0.
 * @param[in]  src  Pointer to the source NUL-terminated string. Must not be NULL.
 ...
 *
 * @note This function is designed to prevent buffer overflows common with `strcpy`.
 *       The behavior is well-defined even if src string is longer than size-1.
 *
 * @example
 * @code
 * char destination[10];
...
size_t kstrlcpy_c23(char * restrict dest, const char * restrict src, size_t size) {
    ...
    if (size > 0) {
        KASSERT(dest != NULL, "kstrlcpy_c23: destination pointer is NULL for non-zero size");
    }
    ...
    if (N == 0) {
        ...
        while (*src++) { // Count remaining chars in 'src' to get total original length
            // No body needed
        }
    }

    return (src - s_orig - 1); // src points one past NUL or where loop stopped, s_orig is start
}
