 ...
 *                  if n > 0.
 * @param[in]  c    The byte value to fill the memory with (converted to unsigned char).
 ...
 * // buffer now contains {'A','A','A','A','A',0,0,0,0,0} (assuming original zeroing)
 * @endcode
 */
void *kmemset_c23(void *dest, int c, size_t n) {
    // KASSERT for destination pointer validity, especially if n > 0.
    // If n is 0, dest can potentially be NULL by some standards, but safer to assert non-NULL if used.
    // However, if n=0, the loop won't run, so NULL dest might be benign.
    // Let's assert dest != NULL only if n > 0, or always for stricter checking.
    // For kernel code, stricter is often better.
    KASSERT(dest != NULL || n == 0, "kmemset_c23: destination pointer is NULL for non-zero n");
