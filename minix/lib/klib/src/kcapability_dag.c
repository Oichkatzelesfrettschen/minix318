 ...
 * performance statistics, and setting up the invariant validation function pointer.
 * This implementation uses a single static instance for the DAG to avoid dynamic
 * kernel memory allocation for the DAG structure itself, ensuring it's available
 * early. The initialization occurs only once.
 *
 * @return Pointer to the initialized (or already initialized) static DAG instance.
 */
kcapability_dag_t* kcapability_dag_create(void) {
    ...
    if (!klib_dag_initialized) {
        ...
        klib_dag_instance.validate_invariants = kcapability_dag_validate_invariants_impl;
...
kcapability_dag_node_t* kcapability_dag_node_create(kcapability_dag_t* dag,
  ...
  // DFS flags (already present, ensure they are initialized by kmemset or explicitly)
...
static kbool kcapability_dag_dfs_for_cycle_check_recursive(
  ...
  for (_BitInt(KCAPABILITY_MAX_EDGES_PER_NODE > 0
    ...
    if (!adjacent_node->visited_flag) {
      ...
      // found a path.
      if (adjacent_node == target_for_cycle) return true;
    ...
    if (adjacent_node == target_for_cycle) return true;  // Path found
  }
...
kbool kcapability_dag_would_create_cycle(kcapability_dag_t* dag,
  ...
  if (source == dest) return true;  // Self-loops are cycles
...
kcapability_dag_status_t kcapability_dag_add_edge(
  ...
  if (parent == child) return KCAP_SELF_REFERENCE;
  ...
  if (child->num_parents >= KCAPABILITY_MAX_PARENTS_PER_NODE) {
      ...
      return KCAP_MAX_EDGES_REACHED; // Re-using this error, though it's for parents here
  }
...
kcapability_dag_status_t kcapability_dag_add_edge_unsafe(
  ...
  if (child->num_parents >= KCAPABILITY_MAX_PARENTS_PER_NODE) {
      ...
      // kpanic("kcapability_dag_add_edge_unsafe: Child node has reached max parents.");
...
kbool kcapability_dag_dfs_validate_path(kcapability_dag_node_t* current,
                                        kcapability_dag_node_t* target,
                                        kbool* visited,
                                        krights_mask_t path_rights_mask) {
  KASSERT(current != NULL && target != NULL && visited != NULL);
  KASSERT(current->is_active);  // Should be called on active nodes

  // Check if current node's rights are a subset of what's allowed by this path
  // from parent
  ...
  // sized KCAPABILITY_MAX_NODES and indices are derivable if DAG ptr was here.
  // For now, this function has a flaw if node indices aren't directly in node
  // struct. Let's assume for a moment node->id could be used if it's a dense 0
  // to N-1 index, which is not true. This helper needs redesigning or the main
  // kcapability_dag_validate_derivation needs to pass indices. TEMPORARY: using
  // current->visited_flag for this DFS run. Requires resetting flags before
  // main call.
...
kbool kcapability_dag_validate_derivation(
  ...
  if (source_capability == target_capability) {  // Deriving itself is valid if
  ...
  // which is reset above. The initial call starts with the source_capability's
  // own rights_mask.
