...
kcapability_dag_validate_invariants_impl(const kcapability_dag_t *dag) {
  ...
  // Basic invariant: if node_capacity is 0, nodes pointer should ideally be
  // NULL, and node_count must be 0. If node_capacity > 0, nodes should not be
  // NULL.
  ...
  // Basic invariant: edge_count should not be negative (it's unsigned _BitInt,
  // so check against large values if it wraps) For _BitInt, it won't be
  // negative. A check for consistency (e.g. sum of all child_count ==
  // edge_count) is more advanced. KASSERT(dag->edge_count >= 0wb); // Implicit
  // for unsigned _BitInt
...
kbool kcapability_dag_dfs_path_exists(kcapability_dag_t *dag,
  ...
  kbool kcapability_dag_dfs_validate_path(
     ...
     * @brief DFS helper to validate a derivation path considering rights masks.
     *
     * Traverses from 'current' to 'target'. Along any path found, it ensures
     * that the rights_mask of each child node is a subset of the
     * 'current_rights_mask' passed down from its parent in that specific path
     * traversal.
     *
     * @param current The current node in the DFS traversal.
     * @param target The target node of the derivation.
     * @param visited An array to keep track of visited nodes for this DFS run.
     * @param path_rights_mask The rights mask allowed by the parent in this
     * path. The 'current' node's actual rights_mask must be a subset of this.
     * @return kbool True if a valid path to target is found respecting rights,
     * false otherwise.
     */
    kbool kcapability_dag_dfs_validate_path(
      ...
      // managed differently, or node struct needs an index. Assuming 'visited'
      // is sized KCAPABILITY_MAX_NODES and indices are derivable if DAG ptr was
      // here. For now, this function has a flaw if node indices aren't directly
      // in node struct. Let's assume for a moment node->id could be used if
      // it's a dense 0 to N-1 index, which is not true. This helper needs
      // redesigning or the main kcapability_dag_validate_derivation needs to
      // pass indices. TEMPORARY: using current->visited_flag for this DFS run.
      // Requires resetting flags before main call. For
      // kcapability_dag_get_node_index, we need the DAG pointer. This
      // function's signature lacks it. This implies 'visited' might need to be
      // managed differently, or node struct needs an index. Assuming 'visited'
      // is sized KCAPABILITY_MAX_NODES and indices are derivable if DAG ptr was
      // here. For now, this function has a flaw if node indices aren't directly
      ...
      // redesigning or the main kcapability_dag_validate_derivation needs to
      // pass indices. TEMPORARY: using current->visited_flag for this DFS run.
      // Requires resetting flags before main call.
     ...
     * kcapability_dag_add_edge. This function primarily focuses on path
     * existence and rights subset property along the path.
     *
     * @param dag The capability DAG.
    ...
    kbool kcapability_dag_validate_derivation(
      ...
      // For kcapability_dag_dfs_validate_path, it needs a 'visited' array if
      // it's to be re-entrant and not rely on global node flags for a single
      // DFS run. The current kcapability_dag_dfs_validate_path uses
      // node->visited_flag, which is reset above. The initial call starts with
      // the source_capability's own rights_mask.
    ...
    void kcapability_dag_validate_invariants_impl(kcapability_dag_t * dag) {
      ...
      // 1. No cycles.
      // 2. num_nodes matches active nodes.
      // 3. For every edge (u,v):
      //    a. v.security_level >= u.security_level
      //    b. v.rights_mask is a subset of u.rights_mask
      //    (These are checked on add_edge, but invariant check confirms
      //    consistency)
      // 4. Node indices are valid.
      // 5. Adjacency counts are correct.
    }
...
#include <stdlib.h> // For kzalloc, kfree (assuming they map to stdlib for now)
    ...
    kcapability_node_t *find_process_capability(void *proc_ptr) {
      ...
      // if(node) { node->id = 1; node->rights = 0xFFFFFFFF; node->creation_time
      // = 0; } return node;
      return NULL;
    }
    ...
    kcapability_node_t **find_capability_path(kcapability_node_t * from,
                                              kcapability_node_t * to,
                                              size_t *path_length) {
      (void)from;
      (void)to;
      (void)path_length;
    ...
    bool is_parent_child_relation(kcapability_node_t * parent,
                                  kcapability_node_t * child) {
      (void)parent;
      (void)child;
      // Dummy implementation
...
#include <stdlib.h> // For kzalloc, kfree (assuming they map to stdlib for now)
    ...
    /**
     * @file kcapability_dag.c
     * @brief Implementation of Capability Directed Acyclic Graph for MINIX
     * kernel
     *
     * This module implements the mathematical foundation for capability-based
     * security through directed acyclic graph operations. Every function here
...
#include <stddef.h>                // For size_t
    ...
    // Forward declaration for the validation function pointer
    static bool kcapability_dag_validate_invariants_impl(
        const kcapability_dag_t *dag);

    // Forward declarations for static helper functions
    static bool kcapability_dag_would_create_cycle(
        kcapability_dag_t * dag, kcapability_dag_node_t * parent,
        kcapability_dag_node_t * child);
    static int kcapability_dag_add_edge_unsafe(
        kcapability_dag_t * dag, kcapability_dag_node_t * parent_node,
        kcapability_dag_node_t * child_node);
    static size_t kcapability_dag_get_node_index(
        const kcapability_dag_t *dag, const kcapability_dag_node_t *node);
    static bool kcapability_dag_dfs_path_exists(
        const kcapability_dag_t *dag, kcapability_dag_node_t *current,
        kcapability_dag_node_t *target, bool *visited);
    static bool kcapability_dag_dfs_validate_path(
        const kcapability_dag_t *dag, kcapability_dag_node_t *current,
        kcapability_dag_node_t *target, bool *visited);
...
#define ERROR_INVALID_ARGUMENT -6 // Example error code
#endif
    // #define ERROR_OUT_OF_MEMORY -4 // Defined in
    // kcapability_dag_add_edge_unsafe if needed

    /**
     * @brief Create a new capability DAG with specified initial capacity
     *
     * This function creates the mathematical foundation for capability
     * management. Think of this like establishing the axioms for a mathematical
     * system - everything else builds upon this foundation.
     */
    kcapability_dag_t *kcapability_dag_create(size_t initial_capacity) {
      /* Validate input parameters using kernel assertions */
      KASSERT(initial_capacity > 0,
              "kcapability_dag_create: initial_capacity must be > 0");
      KASSERT(initial_capacity <= MAX_CAPABILITY_NODES,
              "kcapability_dag_create: initial_capacity exceeds "
     ...
     * This function checks fundamental mathematical properties that must always
     * hold for a consistent DAG structure. For example, node_count should not
     * exceed node_capacity. More sophisticated checks (e.g., full acyclicity
     * check, consistency of parent/child counts) can be added here. This
     * function is crucial for runtime verification of the DAG's integrity.
     *
     * @param dag Pointer to the capability DAG to validate.
     * @return True if all checked invariants hold, false otherwise.
     *
    ...
    static bool kcapability_dag_validate_invariants_impl(
      ...
      for (_BitInt(32) i = 0wb; i < dag->node_count; i++) {
        // Note: using const_cast here if we need to modify dag->nodes[i] for
        // some check, but for read-only validation, direct const
        // kcapability_dag_node_t* is fine. The current dag->nodes is
        // kcapability_dag_node_t*, so direct access is fine.
        ...
        KASSERT(node->child_count <= node->children_capacity,
                "validate_invariants_impl: node ID %llu: child_count (%u) "
                "exceeds children_capacity (%u)",
        ...
        if (node->child_count > 0wb) {
          KASSERT(node->children != NULL,
                  "validate_invariants_impl: node ID %llu has child_count > 0 "
                  "but children pointer is NULL",
                  (unsigned long long)node->capability_id);
          ...
          // If children_capacity > 0 but child_count is 0, children pointer
          // should not be NULL (it's an allocated empty array). This depends on
          // krealloc behavior for size 0 and initial allocation strategy. For
          // now, allow NULL if count is 0. A stricter check might be: if
        ...
        KASSERT(node->parent_count <= node->parents_capacity,
                "validate_invariants_impl: node ID %llu: parent_count (%u) "
                "exceeds parents_capacity (%u)",
                (unsigned long long)node->capability_id,
                (unsigned int)node->parent_count,
                (unsigned int)node->parents_capacity);
        if (node->parent_count > 0wb) {
          KASSERT(node->parents != NULL,
                  "validate_invariants_impl: node ID %llu has parent_count > 0 "
                  "but parents pointer is NULL",
                  (unsigned long long)node->capability_id);
          ...
          // Similar logic for parents pointer as for children pointer if count
          // is 0.
        }

        calculated_edge_count +=
            node->child_count; // Each entry in a children list represents one
                               // directed edge
      }

      // Edge count consistency: Sum of all outgoing edges (all child_count)
      // should equal dag->edge_count.
      ...
      // 2. For each node A, and each child B of A, verify that A is a parent of
      // B.
      // 3. For each node A, and each parent B of A, verify that A is a child of
      // B.
...
#define VALIDATION_UNKNOWN 0 // Assuming an enum or defined constants
    ...
    // Forward declaration for get_kernel_timestamp (assuming it exists or will
    // be stubbed) uint64_t get_kernel_timestamp(void); // Or appropriate type
     ...
     *                       Must be within [MIN_SECURITY_LEVEL,
     * MAX_SECURITY_LEVEL].
     * @return Pointer to the newly created kcapability_dag_node_t, or triggers
     * a kernel panic if allocation fails.
     *
     * @pre capability_id != INVALID_CAPABILITY_ID
     * @pre rights_mask != 0
     * @pre security_level >= MIN_SECURITY_LEVEL && security_level <=
     ...
     * @post node->generation == 1
     */
    kcapability_dag_node_t *kcapability_dag_node_create(
        _BitInt(64) capability_id, _BitInt(64) rights_mask,
        _BitInt(16) security_level, const char *name_param) {
      /* Validate mathematical preconditions - Axiom Check for Node Properties
       */
      KASSERT(capability_id != INVALID_CAPABILITY_ID,
              "kcapability_dag_node_create: capability_id is invalid");
      ...
      // resource_ptr is passed to kcapability_dag_node_create in the header,
      // but not in this C code. The header was:
      // kcapability_dag_node_create(_BitInt(64) capability_id, _BitInt(64)
      // rights_mask, _BitInt(16) security_level); The original template had:
      // kcapability_dag_node_create(_BitInt(64) id, _BitInt(32)
      // granted_permissions, void *resource_ptr); The C code here does not take
      // resource_ptr. Let's assume it's set separately or not needed for this
      // version. For consistency with the current header, I will assume
      // resource_ptr is not set here. If resource_ptr was part of the function
      // signature for this create call, it would be: node->resource_ptr =
      // resource_ptr; However, the feedback's C code for this function does not
      // include resource_ptr as a parameter. The header was changed to match
      // this C code's parameters. The kcapability_dag_node_t struct *does* have
      // resource_ptr. It should be initialized.
      node->resource_ptr = NULL; // Initialize to NULL as it's not passed in
      ...
      node->owner_endpoint = NONE; // Assuming NONE is a defined invalid
                                   // endpoint (e.g. from <minix/endpoint.h>)
     ...
     * @param child_node Pointer to the child capability node.
     * @return SUCCESS (0) if the edge was added successfully.
     * @return CAPABILITY_RIGHTS_VIOLATION if the child's rights are not a
     * subset of the parent's.
     * @return CAPABILITY_SECURITY_VIOLATION if the child's security level is
     * less than the parent's.
     * @return CAPABILITY_CYCLE_DETECTED if adding this edge would create a
     * cycle.
     * @return Other negative error codes on internal failure (e.g., memory
     * allocation).
     *
     * @pre dag, parent_node, and child_node must not be NULL.
     * @pre parent_node and child_node must be different nodes.
     * @post If successful, an edge from parent_node to child_node is added to
     * the DAG.
     * @post dag->edge_count is incremented.
     * @post dag->performance_stats.operations_total is incremented.
     * @post The DAG invariants hold true.
    ...
    int kcapability_dag_add_edge(kcapability_dag_t * dag,
      ...
      KASSERT(parent_node != NULL,
              "kcapability_dag_add_edge: parent_node is NULL");
      KASSERT(child_node != NULL,
              "kcapability_dag_add_edge: child_node is NULL");
      KASSERT(parent_node != child_node,
              "kcapability_dag_add_edge: parent_node and child_node cannot be "
              "the same (self-edges would create cycles)");

      /*
       * Mathematical constraint verification: The child's rights must be
       * a subset of the parent's rights. This enforces the capability
       * monotonicity theorem at runtime.
       * (parent_rights | child_rights) == parent_rights  <=> child_rights is
       * subset of parent_rights
       */
      if ((child_node->rights_mask & parent_node->rights_mask) !=
          child_node->rights_mask) {
        return CAPABILITY_RIGHTS_VIOLATION;
       ...
       */
      int result =
          kcapability_dag_add_edge_unsafe(dag, parent_node, child_node);
      if (result != SUCCESS) {
        return result; // Propagate error from unsafe helper (e.g., out of
                       // memory)
      }

      /* Update DAG-level statistics for mathematical analysis */
      dag->edge_count++; // Assumes edge_count is _BitInt, direct ++ might need
                         // casting or helper if not. For _BitInt, direct
                         // increment is fine.
      dag->performance_stats.operations_total++;

      /* Verify that our mathematical invariants still hold - Post-condition
       * Check */
      if (!dag->validate_invariants(dag)) {
        // This signifies a critical internal error or flaw in logic if
        // invariants fail.
        kpanic("DAG mathematical invariants violated after edge addition");
      }
     ...
     *
     * This function updates the parent's children list and the child's parents
     * list to remove the relationship. It does not destroy the nodes
     * themselves.
     *
     * @param dag Pointer to the capability DAG.
     ...
     * @post parent_node->child_count and child_node->parent_count are
    ...
    int kcapability_dag_remove_edge(kcapability_dag_t * dag,
      ...
      if (!found_in_child) {
        ...
        KASSERT(0, "kcapability_dag_remove_edge: DAG inconsistency - edge "
                   "found in parent but not child");
        // Attempt to restore parent's state (best effort, complex due to
        // potential reallocs not done here) This part is non-trivial to
        // perfectly revert. A production system might need transactions or
        // careful state. For now, panic via KASSERT.
      ...
      KASSERT(dag->validate_invariants(dag),
              "kcapability_dag_remove_edge: DAG invariants failed after edge "
              "removal");
     ...
     * @return SUCCESS if the node was found and deleted, ERROR_NOT_FOUND
     * otherwise.
     *
     * @pre dag must not be NULL.
    ...
    int kcapability_dag_delete_node(kcapability_dag_t * dag, _BitInt(64) id) {
      ...
      KASSERT(
          node_to_delete->child_count == 0wb,
      ...
      KASSERT(
          node_to_delete->parent_count == 0wb,
          "delete_node: parent_count not zero after removing incoming edges");

      // Free the internal children and parents arrays of the node structure
      // being deleted. These arrays contain pointers to other nodes, not the
      // nodes themselves.
      ...
      if (dag->node_count < dag->node_capacity) {
        // Example using a loop if kmemset not available/desired for _BitInt
        // struct members kcapability_dag_node_t* last_slot =
        // &dag->nodes[dag->node_count]; last_slot->capability_id =
        // INVALID_CAPABILITY_ID; last_slot->rights_mask = 0wb;
        // ... and so on for all fields.
        // Or, if a kmemset equivalent is available and safe for this struct:
        // kmemset(&dag->nodes[dag->node_count], 0,
        // sizeof(kcapability_dag_node_t));
      }

      dag->performance_stats.operations_total++;

      KASSERT(dag->validate_invariants(dag),
              "kcapability_dag_delete_node: DAG invariants failed after node "
              "deletion");
    ...
    int kcapability_dag_delete_node(kcapability_dag_t * dag, _BitInt(64) id) {
      ...
      // Remove all outgoing edges (node_to_delete is parent)
      // Iterate backwards over a copy of children if
      // kcapability_dag_remove_edge modifies the list dynamically or ensure
      // remove_edge handles list modification during iteration carefully.
      // Current remove_edge shifts elements, so iterating backwards on count is
      // safer.
      while (node_to_delete->child_count > 0wb) {
      ...
      // Free the internal children and parents arrays of the node being
      // deleted. This is crucial as these were dynamically allocated by
      ...
      if (dag->node_count <
          dag->node_capacity) { // Ensure not to write out of bounds if count
                                // was already capacity
                                // kmemset(&dag->nodes[dag->node_count], 0,
        // sizeof(kcapability_dag_node_t)); // If kmemset is available
      }
    ...
    // Forward declaration for the new DFS helper, if not already present from
    // previous steps (kcapability_dag_dfs_path_exists was for cycle detection,
    // this one is for validation path) Based on user feedback, this is a new
    // DFS specifically for validation. static bool
    // kcapability_dag_dfs_validate_path(const kcapability_dag_t* dag,
    // kcapability_dag_node_t* current, kcapability_dag_node_t* target, bool*
    // visited); This forward declaration is now added above with other static
    // helpers.
     ...
     *
     * This function constructs a mathematical proof that one capability
     * (target_capability) can be validly derived from another
     * (source_capability) through the relationships encoded in the DAG. This
     * involves checking for a valid path and ensuring that all intermediate
     * capability derivation rules (e.g., rights subset, security level
     * monotonicity) are maintained along that path.
     *
     * @param dag Pointer to the capability DAG.
     * @param source_capability The starting capability node in the potential
     * derivation chain.
     * @param target_capability The target capability node whose derivation is
     * being validated.
     * @return True if a valid derivation path exists, false otherwise.
     *
     * @pre dag, source_capability, and target_capability must not be NULL.
     * @pre The DAG's mathematical invariants must hold.
     * @post Performance statistics (successful_derivations/failed_derivations)
     * are updated.
     */
    bool kcapability_dag_validate_derivation(
        kcapability_dag_t * dag, kcapability_dag_node_t * source_capability,
      ...
      bool *visited = (bool *)kcalloc((size_t)dag->node_capacity, sizeof(bool));
      if (!visited && dag->node_capacity >
                          0) { // Check if kcalloc failed for non-zero capacity
        kpanic("kcapability_dag_validate_derivation: failed to allocate "
               "visited array");
        // Defensive return, kpanic might not return
        return false;
      }
      // If dag->node_capacity is 0, visited will be NULL from kcalloc(0, size).
      // dfs_validate_path must handle a NULL visited array if node_capacity is
      // 0. However, kcapability_dag_dfs_validate_path will likely use
      // get_node_index, which assumes nodes exist. The check for node_capacity
      // == 0 above should handle this.
     ...
     * @brief Lookup a capability node by ID with mathematical validation.
     *
     * This function searches for a capability node within the DAG given its
     * unique capability ID. The current implementation uses a linear search
     * through the DAG's node array. It ensures that mathematical invariants of
     * the DAG are checked before and potentially after the operation (though
     * lookup is read-only). Performance statistics for cache hits/misses
     * (conceptually) are updated.
     *
     * @param dag Pointer to the capability DAG.
     * @param capability_id The unique ID of the capability node to find.
     * @return Pointer to the found kcapability_dag_node_t if it exists,
     * otherwise NULL.
     *
     * @pre dag != NULL.
     * @pre capability_id != INVALID_CAPABILITY_ID.
     ...
     * @post Performance statistics (cache_hits/cache_misses) are updated.
     */
    kcapability_dag_node_t *kcapability_dag_lookup_node(
        kcapability_dag_t * dag, _BitInt(64) capability_id) {
      KASSERT(dag != NULL, "kcapability_dag_lookup_node: dag is NULL");
      // INVALID_CAPABILITY_ID should be defined (e.g., in kcapability_dag.c or
      // kcapability_dag.h)
      KASSERT(capability_id != INVALID_CAPABILITY_ID,
      ...
      /*
       * Mathematical approach: Linear search for now, as specified.
       * This assumes dag->nodes is an array of kcapability_dag_node_t
       * structures. The mathematical structure (e.g., if nodes were in a hash
       * table based on ID) would enable more sophisticated indexing strategies
       * for O(1) average case. This will be implemented in future optimization
       * phases.
       */
      for (_BitInt(32) i = 0wb; i < dag->node_count; i++) {
        // Ensure we are not accessing beyond allocated memory if node_count is
    ...
    /**
     * @brief Destroys a single capability DAG node, freeing associated
     * resources.
     *
     * This function frees the memory allocated for the node itself and any
     * dynamically allocated internal structures, such as arrays for parent
     ...
     *      other nodes before being destroyed to maintain DAG consistency,
     *      unless the entire DAG is being torn down.
     * @post All memory directly owned by the node (e.g., children/parents
     * arrays) is freed.
     * @post The memory for the node structure itself is freed.
     */
    void kcapability_dag_node_destroy(kcapability_dag_node_t * node) {
      ...
      node->rights_mask = 0wb;
      node->security_level = 0wb;
      // node->resource_ptr = NULL; // If it points to something the node owns,
      // it should be freed too. If it's just a reference, NULLing it out is
      // fine.
     ...
     *
     * This function iterates through all nodes in the DAG (if the DAG owns them
     * in a flat array structure) and calls kcapability_dag_node_destroy on
     * each. It then frees the main node array and the DAG structure itself.
     *
     * @param dag Pointer to the kcapability_dag_t to destroy.
     *            If NULL, the function does nothing.
     *
     * @pre The DAG should no longer be in use.
     * @post All memory allocated for the DAG and its nodes (if owned by the
     * DAG's flat array) is freed.
     */
    void kcapability_dag_destroy(kcapability_dag_t * dag) {
      if (!dag) {
        return;
      ...
      if (dag->nodes) {
        for (_BitInt(32) i = 0wb; i < dag->node_count; i++) {
          // For each node, free its dynamically allocated children/parents
          // arrays. The node itself is part of the larger dag->nodes
          // allocation.
          kcapability_dag_node_t *current_node = &dag->nodes[i];
          if (current_node->children) {
            kfree(current_node->children);
      ...
      // If node_store_handle is different from dag->nodes and manages other
      // allocations, it should be cleaned up here too. Current assumption is
      // dag->nodes is the main store. dag->node_store_handle = NULL; // This
      // was removed from kcapability_dag_t in a previous step.
    ...
    /**
     * @brief Helper function to get the index of a node in the DAG's node
     * array.
     *
     * This function assumes that the DAG stores its nodes in a contiguous array
     * (`dag->nodes`) and that the node pointer is a valid pointer within this
    ...
    static size_t kcapability_dag_get_node_index(
      ...
      // Ensure node is within the allocated array bounds for safety,
      // though pointer arithmetic itself doesn't check upper bound here.
      KASSERT(node >= dag->nodes, "kcapability_dag_get_node_index: node "
                                  "pointer is before dag->nodes base");

      ptrdiff_t diff = node - dag->nodes;
      // KASSERT(diff >= 0 && (size_t)diff < dag->node_capacity, "Node out of
      // bounds or not in this DAG's primary node array"); The above KASSERT is
      // more robust but requires node_capacity to be accurate for the
      // dag->nodes array. For now, rely on the fact that nodes are from this
      // array.
      return (size_t)diff;
    }
     ...
     * capability nodes.
     *
     * This function recursively traverses the DAG starting from the 'current'
     * node, attempting to find a path to the 'target' node. Unlike a simple
     * path existence check, this function would also incorporate validation of
     * capability theory rules at each step of the path if needed (e.g.,
     * ensuring rights are non-increasing, security levels are non-decreasing
     * along the path from target to source, or source to target depending on
     * derivation direction).
     *
     * For this implementation, as per the user's direct specification for
     * `kcapability_dag_validate_derivation` which calls this, this DFS will
     * primarily focus on path existence from `source_capability` (initial
     * `current`) to `target_capability`. The specific capability theory
     * constraints (rights subset, security level) are checked in
     * `kcapability_dag_add_edge` when forming the DAG, so a simple path
     * existence here implies a valid derivation chain if the DAG is correctly
     * constructed.
     *
     * @param dag Pointer to the DAG (used for get_node_index).
     * @param current The current node in the DFS traversal.
     * @param target The target node we are trying to find a path to.
     * @param visited An array marking visited nodes to prevent infinite loops
     *                and redundant computations during this specific DFS
     * traversal.
     * @return True if a valid path exists from current to target, false
     * otherwise.
     *
     * @pre dag, current, target, and visited must not be NULL (unless
     * dag->node_capacity is 0, then visited can be NULL).
    ...
    static bool kcapability_dag_dfs_validate_path(
      ...
      KASSERT(current != NULL, "dfs_validate_path: current node is NULL");
      KASSERT(target != NULL, "dfs_validate_path: target node is NULL");
      // Visited can be NULL if dag->node_capacity is 0, kcalloc(0, size)
      // returns NULL. kcapability_dag_validate_derivation handles the
      // node_capacity == 0 case before calling this. So, if called, and
      // node_capacity > 0, visited should not be NULL.
      if (dag->node_capacity > 0) {
        KASSERT(visited != NULL, "dfs_validate_path: visited array is NULL for "
                                 "non-zero capacity DAG");
      }
      ...
      /* Mark current node as visited in our proof construction */
      // This requires dag->nodes to be the flat array store and
      // dag->node_capacity to be correct. If dag->node_capacity is 0, this part
      // will not be reached due to checks in caller.
      size_t current_index = kcapability_dag_get_node_index(dag, current);
      // KASSERT(current_index < dag->node_capacity, "dfs_validate_path:
      // current_index out of bounds");

      // If already visited in this specific DFS call, we've been here before
      // down this path; this check is crucial to prevent infinite loops if,
      // hypothetically, the graph had a cycle (though kcapability_dag_add_edge
      // tries to prevent this).
      if (visited[current_index]) {
        return false; // Already visited this node in the current path search
      ...
      for (_BitInt(16) i = 0wb; i < current->child_count; i++) {
        ...
        // No need to check visited[child_index] before recursive call,
        // the recursive call's own check `if (visited[child_index]) return
        // false;` handles it.
        if (kcapability_dag_dfs_validate_path(dag, child_node, target,
                                              visited)) {
          return true; /* Path found through this child */
        }
      }
      ...
      // Backtrack: Unmark current node as visited if we want to allow it to be
      // found via other paths in more complex search algorithms. For simple
      // path existence from A to B, once a path is exhausted without finding B,
      // nodes on that path don't need unmarking for *this specific call* to
      // dfs_validate_path from its direct parent. The `visited` array is for
      // one top-level call to `kcapability_dag_validate_derivation`. So, no
      // explicit unmarking `visited[current_index] = false;` here.
     ...
     * @param visited An array marking visited nodes to prevent infinite loops
     *                and redundant computations during this specific DFS
     * traversal. The caller is responsible for allocating and freeing this
     * array. Indices correspond to node indices from
     * kcapability_dag_get_node_index.
     * @return True if a path exists from current to target, false otherwise.
     *
    ...
    static bool kcapability_dag_dfs_path_exists(
      ...
      // If dag->nodes is a flat array, get_node_index works.
      size_t current_index = kcapability_dag_get_node_index(dag, current);
      // KASSERT(current_index < dag->node_capacity, "dfs_path_exists:
      // current_index out of bounds for visited array");
      visited[current_index] = true;
      ...
      for (_BitInt(16) i = 0; i < current->child_count; i++) {
        ...
        KASSERT(child_node != NULL,
                "dfs_path_exists: child_node in children array is NULL");
        size_t child_index = kcapability_dag_get_node_index(dag, child_node);
        // KASSERT(child_index < dag->node_capacity, "dfs_path_exists:
        // child_index out of bounds for visited array");

        /* Avoid infinite recursion in proof construction if already visited in
         * *this specific DFS path search* */
        if (!visited[child_index]) {
          if (kcapability_dag_dfs_path_exists(dag, child_node, target,
                                              visited)) {
            return true; /* Proof found through this path */
          }
        }
    ...
    /**
     * @brief Determine if adding an edge would create a cycle using
     * mathematical proof.
     *
     * This function implements mathematical cycle detection through depth-first
     * traversal. The mathematical approach ensures that we can prove the
     * absence of cycles rather than just testing for their presence before
     * adding an edge. An edge parent -> child would create a cycle if there is
     * already a path from child -> parent.
     *
     * @param dag Pointer to the capability DAG.
     * @param parent The proposed parent node of the new edge.
     * @param child The proposed child node of the new edge.
     ...
     *
     * @pre dag, parent, and child must not be NULL.
     * @pre The current graph (before adding parent->child) is assumed to be a
     * DAG.
     */
    static bool kcapability_dag_would_create_cycle(
        kcapability_dag_t * dag, kcapability_dag_node_t * parent,
      ...
      /* Allocate visited tracking for mathematical proof construction.
       * Size based on node_capacity as indices are derived from the base
       * dag->nodes array. kcalloc initializes to false (0).
       */
      if (dag->node_capacity ==
          0) { // Should not happen if dag is properly initialized with capacity
        return false; // Or panic, as a DAG with 0 capacity for nodes is
      ...
      if (!visited) {
        // This is a critical failure. In a real kernel, might try to recover or
        // log. For now, per user spec for allocation failures:
        kpanic("kcapability_dag_would_create_cycle: failed to allocate visited "
               "array");
        return true; // Defensive: assume cycle if we can't check
      }
     ...
     *
     * This function performs the actual pointer manipulations to establish the
     * parent-child relationship by updating their respective children and
     * parents arrays. It handles dynamic resizing of these arrays if necessary.
     * This function does NOT perform cycle checks or other high-level DAG
     * validation; it assumes callers have already done so.
     *
     * @param dag Pointer to the capability DAG (currently unused here, but good
     * for context).
     * @param parent_node Pointer to the parent node.
     * @param child_node Pointer to the child node.
     * @return SUCCESS (0) if the edge was added.
     * @return ERROR_OUT_OF_MEMORY if memory allocation for resizing arrays
     * fails.
     *
     * @note This is a low-level helper. Prefer kcapability_dag_add_edge for
     * general use.
     * @pre parent_node and child_node are valid, non-NULL pointers.
     * @pre parent_node != child_node.
     * @pre Adding this edge does not create a cycle (caller responsibility).
    ...
    static int kcapability_dag_add_edge_unsafe(
      ...
      if (parent_node->child_count >= parent_node->children_capacity) {
        ...
        kcapability_dag_node_t **new_children_array =
            (kcapability_dag_node_t **)krealloc(
                parent_node->children, (size_t)new_capacity_children *
                                           sizeof(kcapability_dag_node_t *));
        if (!new_children_array) {
          return ERROR_OUT_OF_MEMORY;
        }
      ...
      if (child_node->parent_count >= child_node->parents_capacity) {
        ...
        kcapability_dag_node_t **new_parents_array =
            (kcapability_dag_node_t **)krealloc(
                child_node->parents, (size_t)new_capacity_parents *
                                         sizeof(kcapability_dag_node_t *));
        if (!new_parents_array) {
          // Attempt to revert the addition to parent's children list for
          // consistency, though this is tricky if krealloc modified the
          // original block in place before failing. A more robust strategy
          // might be needed for transactional semantics. For now, simple
          // revert:
          parent_node->child_count--;
          return ERROR_OUT_OF_MEMORY;
        }
      ...
      (void)dag; // dag parameter currently unused but kept for API consistency
                 // or future use
