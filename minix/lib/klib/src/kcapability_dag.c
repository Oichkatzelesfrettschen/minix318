...
static bool kcapability_dag_validate_invariants_impl(const kcapability_dag_t* dag) {
    ...
    if (dag->node_capacity == 0) {
        ...
        // If capacity > 0, dag->nodes should have been allocated by kcapability_dag_create.
    ...
    if (dag->node_count == 0) {
         KASSERT(dag->edge_count == 0wb, "validate_invariants_impl: edge_count non-zero for zero nodes");
    }
    ...
    for (_BitInt(32) i = 0wb; i < dag->node_count; i++) {
        // Note: using const_cast here if we need to modify dag->nodes[i] for some check,
        // but for read-only validation, direct const kcapability_dag_node_t* is fine.
        // The current dag->nodes is kcapability_dag_node_t*, so direct access is fine.
        const kcapability_dag_node_t* node = &dag->nodes[i];

        KASSERT(node->capability_id != INVALID_CAPABILITY_ID, "validate_invariants_impl: node %lu has INVALID_CAPABILITY_ID", (unsigned long)i);

        // Check node's internal capacities and pointers for children
        KASSERT(node->child_count <= node->children_capacity,
                "validate_invariants_impl: node ID %llu: child_count (%u) exceeds children_capacity (%u)",
                (unsigned long long)node->capability_id, (unsigned int)node->child_count, (unsigned int)node->children_capacity);
        if (node->child_count > 0wb) {
            KASSERT(node->children != NULL,
                    "validate_invariants_impl: node ID %llu has child_count > 0 but children pointer is NULL", (unsigned long long)node->capability_id);
            for(_BitInt(16) c_idx = 0wb; c_idx < node->child_count; ++c_idx) {
                KASSERT(node->children[c_idx] != NULL,
                        "validate_invariants_impl: node ID %llu has NULL entry in children list at index %u", (unsigned long long)node->capability_id, (unsigned int)c_idx);
            }
        } else { // child_count is 0
            // If children_capacity is also 0, children pointer can be NULL.
            // If children_capacity > 0 but child_count is 0, children pointer should not be NULL (it's an allocated empty array).
            // This depends on krealloc behavior for size 0 and initial allocation strategy.
            // For now, allow NULL if count is 0. A stricter check might be:
            // if (node->children_capacity > 0) KASSERT(node->children != NULL, "...");
        }

        // Check node's internal capacities and pointers for parents
        KASSERT(node->parent_count <= node->parents_capacity,
                "validate_invariants_impl: node ID %llu: parent_count (%u) exceeds parents_capacity (%u)",
                (unsigned long long)node->capability_id, (unsigned int)node->parent_count, (unsigned int)node->parents_capacity);
        if (node->parent_count > 0wb) {
            KASSERT(node->parents != NULL,
                    "validate_invariants_impl: node ID %llu has parent_count > 0 but parents pointer is NULL", (unsigned long long)node->capability_id);
            for(_BitInt(16) p_idx = 0wb; p_idx < node->parent_count; ++p_idx) {
                KASSERT(node->parents[p_idx] != NULL,
                        "validate_invariants_impl: node ID %llu has NULL entry in parents list at index %u", (unsigned long long)node->capability_id, (unsigned int)p_idx);
            }
        } else { // parent_count is 0
            // Similar logic for parents pointer as for children pointer if count is 0.
        }

        calculated_edge_count += node->child_count; // Each entry in a children list represents one directed edge
    }

    // Edge count consistency: Sum of all outgoing edges (all child_count) should equal dag->edge_count.
    KASSERT(dag->edge_count == calculated_edge_count,
            "validate_invariants_impl: dag->edge_count (%llu) does not match calculated sum of node child_counts (%llu)",
            (unsigned long long)dag->edge_count, (unsigned long long)calculated_edge_count);
    ...
    //    (Already done by kcapability_dag_add_edge before adding edges, but a full check here could verify overall integrity).
    ...
    // 4. Validate all node indices if a flat array model is strictly used and nodes can move. (Not an issue with current delete).
 ...
 * @post parent_node->child_count and child_node->parent_count are decremented accordingly.
...
int kcapability_dag_remove_edge(kcapability_dag_t* dag,
    ...
    KASSERT(parent_node != NULL, "kcapability_dag_remove_edge: parent_node is NULL");
    KASSERT(child_node != NULL, "kcapability_dag_remove_edge: child_node is NULL");

    bool found_in_parent = false;
    _BitInt(16) i; // Declare loop variable outside for C89 compatibility if needed, but C23 is fine.
    ...
    for (i = 0wb; i < parent_node->child_count; i++) {
        if (parent_node->children[i] == child_node) {
            ...
            if (parent_node->child_count > 0 || i == 0wb) { // Avoid access if count became 0 and i was 0.
                 if (parent_node->child_count < parent_node->children_capacity) // Check bound before NULLing
                    parent_node->children[parent_node->child_count] = NULL; // Clear last (now unused) element
            } else if (parent_node->child_count == 0wb && parent_node->children_capacity > 0wb) { // If list becomes empty
                 parent_node->children[0] = NULL; // Clear the first element if it was the only one
            }
    ...
    for (i = 0wb; i < child_node->parent_count; i++) {
        if (child_node->parents[i] == parent_node) {
            ...
            if (child_node->parent_count > 0 || i == 0wb) {
                if (child_node->parent_count < child_node->parents_capacity) // Check bound
                    child_node->parents[child_node->parent_count] = NULL; // Clear last (now unused) element
            } else if (child_node->parent_count == 0wb && child_node->parents_capacity > 0wb) {
    ...
    if (!found_in_child) {
        ...
        KASSERT(0, "kcapability_dag_remove_edge: DAG inconsistency - edge found in parent but not child");
        // Attempt to restore parent's state (best effort, complex due to potential reallocs not done here)
        // This part is non-trivial to perfectly revert. A production system might need transactions or careful state.
        // For now, panic via KASSERT.
    ...
    KASSERT(dag->validate_invariants(dag), "kcapability_dag_remove_edge: DAG invariants failed after edge removal");
...
int kcapability_dag_delete_node(kcapability_dag_t* dag, _BitInt(64) id) {
    ...
    KASSERT(id != INVALID_CAPABILITY_ID, "kcapability_dag_delete_node: invalid capability ID");
    ...
    while (node_to_delete->child_count > 0wb) {
        ...
        KASSERT(child_node != NULL, "delete_node: NULL child found in outgoing edges");
        kcapability_dag_remove_edge(dag, node_to_delete, child_node);
    }
    KASSERT(node_to_delete->child_count == 0wb, "delete_node: child_count not zero after removing outgoing edges");
    ...
    while (node_to_delete->parent_count > 0wb) {
        ...
        KASSERT(parent_node != NULL, "delete_node: NULL parent found in incoming edges");
        kcapability_dag_remove_edge(dag, parent_node, node_to_delete);
    }
    KASSERT(node_to_delete->parent_count == 0wb, "delete_node: parent_count not zero after removing incoming edges");

    // Free the internal children and parents arrays of the node structure being deleted.
    // These arrays contain pointers to other nodes, not the nodes themselves.
    ...
    // Optionally, clear the last (now unused) slot in dag->nodes to prevent dangling data.
    // This is good practice if the memory might be inspected later.
    if (dag->node_count < dag->node_capacity) {
         // Example using a loop if kmemset not available/desired for _BitInt struct members
         // kcapability_dag_node_t* last_slot = &dag->nodes[dag->node_count];
    ...
    KASSERT(dag->validate_invariants(dag), "kcapability_dag_delete_node: DAG invariants failed after node deletion");
...
int kcapability_dag_delete_node(kcapability_dag_t* dag, _BitInt(64) id) {
    ...
    // Remove all outgoing edges (node_to_delete is parent)
    // Iterate backwards over a copy of children if kcapability_dag_remove_edge modifies the list dynamically
    // or ensure remove_edge handles list modification during iteration carefully.
    // Current remove_edge shifts elements, so iterating backwards on count is safer.
    while (node_to_delete->child_count > 0wb) {
    ...
    // This is crucial as these were dynamically allocated by kcapability_dag_add_edge_unsafe.
    if (node_to_delete->children) {
        kfree(node_to_delete->children);
        node_to_delete->children = NULL;
        node_to_delete->children_capacity = 0wb;
    ...
    if (dag->node_count < dag->node_capacity) { // Ensure not to write out of bounds if count was already capacity
         // kmemset(&dag->nodes[dag->node_count], 0, sizeof(kcapability_dag_node_t)); // If kmemset is available
    }
