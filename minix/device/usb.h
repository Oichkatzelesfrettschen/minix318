...
/** Associate user data with a device. */
...
/** Obtain the device identification descriptor. */
...
/** Cancel a previously submitted USB request block. */
int ddekit_usb_cancle_urb(struct ddekit_usb_urb *d_urb);

/** Generic information query helper. */
long ddekit_usb_info(struct ddekit_usb_dev *, long, long);

/**
 * Initialize the USB subsystem.
 *
 * Only implemented for the client side; server support must be provided
 * by the DDELinux/FBSD layer.
 */
int ddekit_usb_init(struct ddekit_usb_driver *drv, ddekit_usb_malloc_fn *_m,
